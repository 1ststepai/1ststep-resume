...
- `storage`: holds only a job-posting handoff for up to 24 hours and the non-sensitive current detected job; never profile, résumé, auth token, answers, or application status. A job the user sends to 1stStep.ai is separately saved server-side under their account as an encrypted record that expires after 90 days; that record is disclosed in the privacy policy and is exportable and deletable by the user. Uninstalling the extension does not delete it.
