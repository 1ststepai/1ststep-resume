...
- `storage`: holds only a 15-minute job-capture handoff and the non-sensitive current detected job; never profile, résumé, auth token, answers, or application status.
