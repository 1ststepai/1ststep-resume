...
async function run({signedIn=true, enabled=true, failure=false, mode='', search=''}={}) {
  ...
  const clerk={session:signedIn?{getToken:async(options)=> {assert.equal(options.skipCache,true);return 'fixture.session.token';}}:null,load:async()=>{},signOut:async()=>{result.signedOut=true;},redirectToSignIn:async(options)=>{assert.match(options.signInForceRedirectUrl,/^https:\/\/app\.1ststep\.ai\/login\.html\?returnTo=/);result.signIn=true;},redirectToSignUp:async()=>{result.signUp=true;}};
  await initializeLoginPage({
    ...
    locationRef:{origin:'https://app.1ststep.ai',search:search || (mode?`?mode=${mode}`:'?redirect=https://evil.example'),replace:url=>{result.redirect=url;},reload:()=>{result.reloaded=true;}},
...
const partner=await run({search:'?mode=sign-up&returnTo=%2Fpartner&ref=career-coach'});
assert.equal(partner.redirect,'/partner');
assert.equal(partner.calls[2].url,'/api/partner?action=attribute');
assert.equal(JSON.parse(partner.calls[2].options.body).code,'career-coach');
const unsafe=await run({search:'?returnTo=https%3A%2F%2Fevil.example&ref=%2Fbad'});
assert.equal(unsafe.redirect,'/app');
