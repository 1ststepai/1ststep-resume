...
const files = Object.fromEntries(
  await Promise.all(
    paths.map(async (path) => [
      path,
      await readFile(
        new URL(`../1ststep-extension/${path}`, import.meta.url),
        "utf8",
      ),
    ]),
  ),
...
const storeScreenshots = {
  assistedFill: await readFile(
    new URL("../1ststep-extension/screenshot_3_ats_grid.html", import.meta.url),
    "utf8",
  ),
  workflow: await readFile(
    new URL("../1ststep-extension/screenshot_5_how_it_works.html", import.meta.url),
    "utf8",
  ),
};
...
for (const requiredBlock of [
  ...
  assert.ok(
    files["content.js"].toLowerCase().includes(requiredBlock),
    `autofill must block ${requiredBlock}`,
  );
}
...
assert.doesNotMatch(
  combined,
  /1ststep_profile|1ststep_resume|GET_AUTOFILL_MAP|autofillContext|tierToken/,
);
assert.doesNotMatch(
  files["background.js"],
  /api\/claude|api\/subscription|TRACK_EVENT|track-event/,
);
...
assert.match(
  files["popup.js"],
  /JOB_AGENT_APP_BRIDGE_UNAVAILABLE[\s\S]*Reconnect Agent/,
  "a stale app bridge must be shown as a reconnect problem, not silently downgraded to Resume Tools",
);
assert.match(
  files["popup.html"],
  /id="agentConnectionHint"[\s\S]*aria-live="polite"/,
...
for (const ats of ["workday", "lever", "ashby", "smartrecruiters"]) {
  assert.match(
    ...
    new RegExp(`id: '${ats}'`),
    `generic capture must include the ${ats} adapter`,
  );
}
assert.match(
  files["generic-capture.js"],
...
assert.doesNotMatch(
  files["generic-capture.js"],
  /fetch\(|XMLHttpRequest|chrome\.storage/,
...
const conciergeJs = await readFile(
  new URL("../concierge.js", import.meta.url),
  "utf8",
);
assert.match(
  files["background.js"],
  /\$\{APP_URL\}\/app\/resume\?jobCaptureId=/,
  "job capture must open /app/resume",
);
assert.doesNotMatch(
  files["background.js"],
  /\$\{APP_URL\}\/app\?jobCaptureId=/,
...
assert.match(
  appJs,
  /Nothing is generated until you click Tailor My Resume/,
  "Resume Builder must tell the user that capture does not start AI generation",
);
assert.doesNotMatch(
  appJs,
  /function showJobCaptureConfirm[\s\S]{0,1800}runTailoring\(/,
  "showing a captured job must never spend a credit or start tailoring",
);
assert.match(
  appJs,
  /jobTitle: _srcJob\?\.title \|\| window\._capturedJob\?\.title \|\| kwData\?\.job_title/,
...
assert.match(manifest.description, /supported pages/i);
assert.match(manifest.description, /never submits/i);
assert.doesNotMatch(storeScreenshots.assistedFill, /Greenhouse/i);
assert.match(storeScreenshots.assistedFill, /Review approved details/i);
assert.match(storeScreenshots.assistedFill, /Fill supported fields/i);
assert.match(storeScreenshots.assistedFill, /You submit/i);
assert.match(storeScreenshots.workflow, /Capture a supported job page/i);
assert.match(storeScreenshots.workflow, /You submit\./i);
assert.doesNotMatch(
  Object.values(storeScreenshots).join("\n"),
  /apply to every job|fills every field|submitted\s*[—-]\s*in under a minute/i,
);

assert.match(
  files["content.js"],
  /sendResponse\(\{ success: true, reviewRequired: true, matchAssessment, filled: 0, submitted: false/,
);
assert.ok(
  files["content.js"].indexOf("reviewRequired: true") <
    files["content.js"].indexOf("if (await fillApprovedResume"),
  "match review must happen before document or ordinary-field mutation",
);
assert.match(
  files["content.js"],
  /confirmPrecision: true|msg\.confirmPrecision === true/,
);
assert.match(files["popup.js"], /btn\.dataset\.precisionReviewed = 'true'/);
assert.match(files["popup.html"], /Review match &amp; fill/);

console.log(
  "Controlled Greenhouse extension uses transient server-authorized values and integrity-checked resume bytes, requires match evidence and explicit review before fill, stores no raw profile or document data, performs no AI field guessing, keeps narrow hosts, and never submits.",
);
