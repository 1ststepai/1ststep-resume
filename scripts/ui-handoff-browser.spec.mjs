...
test('mobile subscriber workspace keeps the five-item app navigation in flow', async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 844 });
  await page.goto(`${base}/concierge?uiFixture=subscriber`);
  ...
  await expect(page.locator('.agent-header nav button:visible')).toHaveCount(5);
