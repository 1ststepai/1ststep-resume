...
test('a signed-in user gives one-time scoped authorization before any agent run starts', async ({ page }) => {
  ...
  await expect(page.locator('#jobAgentConsentChecks label')).toContainText('I still review before anything is sent.');
...
test('Learning Center renders only persisted confirmed rules and remains usable on mobile', async ({ page }) => {
  ...
  await expect(page.locator('.agent-header')).toHaveCSS('background-color', 'rgb(23, 23, 23)');
  await expect(page.locator('#agentProgress')).toHaveAttribute('open', '');
  await expect(page.locator('#learningCenter summary')).toBeVisible();
