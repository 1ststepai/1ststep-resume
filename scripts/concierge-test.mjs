...
assert.match(conciergeHtml, /persistent-concierge\.css\?v=20260918-action-first/, 'Job Agent must refresh the stylesheet containing the integrated Agent Status and trust fixes');
...
assert.match(conciergeHtml, /next role\./);
...
assert.match(conciergeJs, /async function requestGuidedLaunch[\s\S]*!sessionCapabilities\.checked \|\| !accountWorkflowHydrated[\s\S]*!hasApiSession\(\) \|\| !hasJobAgentAccess\(\)[\s\S]*ensureJobAgentConsent/);
assert.match(conciergeJs, /hasApiSession\(\) && !hasJobAgentAccess\(\)\) openAgentAccess\(\)/, 'beta eligibility must be surfaced immediately after authenticated hydration');
...
assert.match(conciergeHtml, /id="pathScanPhase"[\s\S]*id="retryOpportunityPaths"/);
assert.match(conciergeJs, /Usually 20–35 seconds[\s\S]*timeout at 40 seconds/);
assert.match(conciergeJs, /X-Correlation-Id/);
assert.match(conciergeHtml, /id="neverIncludeTitle">Never include</);
assert.match(conciergeJs, /exclusions: normalizeMissionExclusions\(guidedSelection\.exclusions\)/);
assert.match(conciergeJs, /reconcileConfirmedResumeFacts/);
assert.match(conciergeJs, /This edit excludes confirmed facts/);
assert.doesNotMatch(conciergeJs, /chooseGuidedGoal\(button, true\)|chooseGuidedLaunchChoice\(button, true\)|setTimeout\(advanceGuidedLaunch, 160\)/, 'guided selections must use an intentional Continue action');
assert.doesNotMatch(conciergeJs, /guidedSelection\.pathId = OPPORTUNITY_PATHS\.find\(path => path\.sector === opportunitySector\)/, 'sector exploration must not silently select a role');
assert.doesNotMatch(`${conciergeHtml}\n${conciergeJs}`, /you\.Add|No matches jobs yet|No interviews jobs yet/);
