...
assert.match(html, /Actual founder result · LogicSource/);
assert.match(html, /This is how much time I actually saved/);
assert.match(html, /This task got me a live phone interview with LogicSource\./);
assert.match(html, /aria-label="35 minutes saved on Evan's LogicSource application task"/);
...
assert.equal(structuredData['@graph'].find(item => item['@type'] === 'FAQPage').mainEntity.length, 9);
...
assert.match(css, /@keyframes time-proof-fill/);
assert.match(css, /@keyframes time-proof-sweep/);
assert.match(css, /\.motion-running \.time-proof-meter/);
