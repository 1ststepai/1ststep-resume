...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { accountExports, artifacts, followUps, runs, sessions, browserTasks, submissionTasks, receiptTasks, vault, campaign, consent, schedule, notifications, residualKeys } = await deleteTenantJobAgentOperationalData({ config, subject: auth.subject });
    const revoked = await revokeAllUserSessions({ ...config, subject: auth.subject });
    clearAccessSessionCookie(res);
    console.info(JSON.stringify({ type: 'account-data-deletion', outcome: 'completed', deletedAccountExports: accountExports.deleted, deletedRuns: runs.deleted, deletedArtifacts: artifacts.deleted, deletedApplicationSessions: sessions.deleted, deletedFollowUpReminders: followUps.deleted, deletedResidualIdempotencyKeys: residualKeys.deleted, deletedEmployerBrowserTasks: browserTasks.deleted, deletedApplicationSubmissionTasks: submissionTasks.deleted, deletedApplicationReceiptTasks: receiptTasks.deleted, deletedApplicantVault: vault.deleted === true, deletedCampaignState: campaign.deleted === true, deletedConsent: consent.deleted === true, deletedSchedule: schedule.deleted === true, deletedNotificationPreference: notifications.deleted === true, deletedNotificationAuxiliaryRecords: Math.max(0, Number(notifications.auxiliaryRecordsDeleted) || 0), deletedEmployerBrowserSessions: browserSessions.deleted, revokedSessions: revoked.revoked }));
