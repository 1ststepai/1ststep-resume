...
| 2 | APIs and backend logic | Input validation, auth, idempotency, safe errors, bounded dependencies; `api/`, `lib/` | Local tests pass across 41 routes. Exact candidate `0b3e5ba` on protected Preview `dpl_8Jn77zQeCGYydjBckKAfYsFMoxYV` returned liveness 200, readiness 503 fail-closed, app configuration 200, and three bounded discovery-canary 200s. Each canary attempted eight representative sources across four ATS providers out of the 37-source catalog and completed inside the function in 1.283-3.055 seconds; one run completed seven sources and reported that truthfully. The full user discovery path has separately bounded provider work and a 45-second ceiling. Production is not the candidate artifact | Warning | Configure an isolated signed-beta environment, run safe signed-user Preview probes, and repeat complete bounded Production acceptance only after release approval. |
| 3 | Database and storage | Tenant isolation, least privilege, RLS/grants, encryption, retention, backup/restore; `migrations/`, tenant stores, object storage | One CLI-generated canonical migration is reconciled from three hashed sources; 20/20 tables enable/force RLS; explicit anon/authenticated revokes, operation policies, and tenant-scoped parent references pass static tests. Candidate `b5bccef` adds a content-free no-network preflight bound to the canonical digest. An authenticated read-only Supabase inventory found two healthy active projects for another product, zero branches, and zero eligible 1stStep.ai targets. The sole visible organization is on the Free Plan and its documented two-project free allocation is already occupied; no schema or data was inspected. The environment still lacks a CLI/container target and live schema/restore remain unknown | **Critical** | Select the organization and authorize an exact cost lookup, or approve a local-runtime installation path; then choose the new-project or paid data-less branch route, separately authorize creation and migration, pass adversarial pgTAP/advisors, verify private object storage, and perform a separately authorized isolated restore. |
| 4 | Authentication and permissions | Opaque sessions, revocation, least privilege, admin/tenant boundaries, consent | Local session, ownership, entitlement, and consent tests pass. Exact Preview `dpl_8Jn77zQeCGYydjBckKAfYsFMoxYV` verified a server-only readiness check exposes only `restoreAccessAvailable: false`. A value-blind Production audit found zero of five consent-control names present and zero of eight controlled-beta names present; actual values and signed-user behavior remain unavailable and unclaimed | Warning | Configure the isolated signed-beta runtime, then verify a least-privilege staging account matrix, email-code session creation, revocation, admin denial, and tenant-crossing denial using synthetic accounts and the exact release commit. |
| 5 | Hosting and deployment | Reproducible clean build, intended routes/assets only, source exclusions, deployment identity | Clean release candidate `0b3e5ba` passed the complete release gate and clean-candidate preflight. Exact protected Preview `dpl_8Jn77zQeCGYydjBckKAfYsFMoxYV` verified route behavior, the representative discovery canary, bounded capacity behavior, and content-free provider logs. Fresh Production parity still fails for all four concierge artifacts and Production lacks the candidate health routes | Warning | Push the reviewed candidate, require green remote CI, resolve Critical infrastructure gates, then obtain explicit Production approval before promotion and bounded acceptance checks. |
| 6 | Cloud and compute | Function limits, isolation, egress, spend, failure boundaries, vendor quotas | Duration, cost, isolation, and fail-closed contracts pass locally. Exact account/project inspection confirms an active Pro plan, Node 24.x, `iad1` function/default region, Fluid Compute, and a standard fixed build machine. Candidate `0b3e5ba` retains the regression-checked 45-second signed-user discovery ceiling; three exact-Preview canaries completed inside the function in 1.283-3.055 seconds. A value-blind Production audit found all seven durable-runtime names but only 14 of 24 cost-control names; values and runtime behavior are not inferred | Warning | Approve explicit function/account quota, queue, latency, and cost ceilings, then perform bounded signed-user dependency-failure and saturation exercises. |
...
| 11 | Load balancing and scaling | Bounded concurrency, backpressure, tenant fairness, capacity assumptions, dependency isolation | Provider concurrency bounds and circuit breakers pass local tests. Exact protected Preview `dpl_8Jn77zQeCGYydjBckKAfYsFMoxYV` on the confirmed Pro/Fluid/`iad1` runtime passed a GET-only, body-free authenticated CLI probe: 10/10 HTTP 200 at concurrency 2, p50 115 ms and p95/max 232 ms, with no bypass secret or writes. The representative discovery canary returned 3/3 HTTP 200 with truthful source counts | Warning | Approve quota, queue, latency, and cost ceilings, then run separately approved signed-user queue/fairness, backpressure, saturation, and dependency-failure exercises; do not infer Production capacity from this bounded liveness sample. |
| 12 | Error tracking and logs | Redaction, structured events, retention, correlation, alert delivery, ownership | Content-free health/events/alert queues pass local tests. Candidate `0b3e5ba` retains fail-closed alerting approval, retention, and acknowledgement-window controls and adds a value-blind environment-name audit. Exact Preview logs contained seven allowlisted expected records with no content-bearing or unexpected record. Production has zero of eight operator-alerting names and zero of eight support-ownership names present; values were not read. Read-only account inspection found zero drains and one uninspected default team-wide alert rule, which is not delivery evidence | Warning | Name owners and approve the exact destination, retention, and acknowledgement window; then verify receiver-side synthetic alert acknowledgement and any approved log retention/dashboard. |
