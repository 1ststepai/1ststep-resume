...
|---|---|---|---|---|---|
| 1 | Frontend foundations | Accessible, responsive, private-data-safe UI; `*.html`, client JS/CSS, browser suite | Candidate `150d15c` passed bounded desktop/mobile interaction and semantic checks. Exact candidate `45afe33` retains that tested UI and its protected Preview `dpl_ELeJmsXYJFZdKP976hHjBcSUDkyb` additionally verified that unavailable sign-in disables the email and send-code controls, states that no code was sent, and emits no console error/warning | Warning | Retain a full automated accessibility and performance artifact, then repeat bounded route, console, responsive, and signed-user checks after Production approval. |
| 2 | APIs and backend logic | Input validation, auth, idempotency, safe errors, bounded dependencies; `api/`, `lib/` | Local tests pass across 41 routes. Exact candidate `45afe33` on protected Preview `dpl_ELeJmsXYJFZdKP976hHjBcSUDkyb` returned liveness 200, readiness 503 fail-closed, a secret-free authentication availability contract from app configuration 200, and concierge smoke 200. Production correctly denies protected unauthenticated APIs but is not the candidate artifact | Warning | Configure an isolated signed-beta environment, run safe signed-user Preview probes, and repeat complete bounded Production acceptance only after release approval. |
