...
|---|---|---|---|---|---|
| 1 | Frontend foundations | Accessible, responsive, private-data-safe UI; `*.html`, client JS/CSS, browser suite | Candidate `150d15c` passed bounded desktop/mobile interaction and semantic checks. Exact candidate `86ac74a` retains that tested UI, and protected Preview `dpl_2ZhDmYkX6UV7Uo2qRWmrNdf9w1vB` verified that unavailable sign-in disables the email and send-code controls and states that no code was sent | Warning | Retain a full automated accessibility and performance artifact, then repeat bounded route, console, responsive, and signed-user checks after Production approval. |
| 2 | APIs and backend logic | Input validation, auth, idempotency, safe errors, bounded dependencies; `api/`, `lib/` | Local tests pass across 41 routes. Exact candidate `86ac74a` on protected Preview `dpl_2ZhDmYkX6UV7Uo2qRWmrNdf9w1vB` returned liveness 200, readiness 503 fail-closed, a secret-free authentication availability contract from app configuration 200, and concierge smoke 200. Its injected Redis configuration passed without process-environment fallback warnings. Production correctly denies protected unauthenticated APIs but is not the candidate artifact | Warning | Configure an isolated signed-beta environment, run safe signed-user Preview probes, and repeat complete bounded Production acceptance only after release approval. |
...
| 11 | Load balancing and scaling | Bounded concurrency, backpressure, tenant fairness, capacity assumptions, dependency isolation | Provider concurrency bounds and circuit breakers pass local tests. Exact protected Preview `dpl_A8rpbCDYU8e9LPEKhLogHS5KQ2zu` passed a GET-only, body-free authenticated CLI probe: 10/10 HTTP 200 at concurrency 2, p50 100 ms and p95/max 134 ms, with no bypass secret or writes. Content-free provider logs corroborated 11 liveness 200s including verification | Warning | Record plan/region quotas, then run separately approved signed-user queue/fairness, backpressure, saturation, and dependency-failure exercises; do not infer Production capacity from this bounded liveness sample. |
