...
|---|---|---|---|---|---|
| 1 | Frontend foundations | Accessible, responsive, private-data-safe UI; `*.html`, client JS/CSS, browser suite | Clean candidate `150d15c` passed the release and browser suites. Protected Preview `dpl_1234TcPAayhdctQZ4g8c64jCqoVY` passed bounded 1440x900 and 375x844 interaction checks with no horizontal overflow or console errors/warnings; the real onboarding opened, a mobile navigation readability defect was corrected, and a scoped semantic audit found no duplicate IDs, unnamed controls, missing image alt text, or positive tab stops | Warning | Retain a full automated accessibility and performance artifact, then repeat bounded route, console, responsive, and signed-user checks after Production approval. |
| 2 | APIs and backend logic | Input validation, auth, idempotency, safe errors, bounded dependencies; `api/`, `lib/` | Local tests pass across 41 routes. Exact candidate `150d15c` on protected Preview `dpl_1234TcPAayhdctQZ4g8c64jCqoVY` returned liveness 200, readiness 503 fail-closed, app configuration 200, and concierge smoke 200. Production correctly denies protected unauthenticated APIs but is not the candidate artifact | Warning | Configure an isolated signed-beta environment, run safe signed-user Preview probes, and repeat complete bounded Production acceptance only after release approval. |
