...
| 2 | APIs and backend logic | Input validation, auth, idempotency, safe errors, bounded dependencies; `api/`, `lib/` | Local tests pass across 41 routes; Preview liveness and live-discovery smoke passed with durable limiting and submissions disabled, while readiness correctly failed closed | Warning | Configure an isolated signed-beta environment, run safe signed-user Preview probes, and perform bounded Production acceptance only after release approval. |
...
| 5 | Hosting and deployment | Reproducible clean build, intended routes/assets only, source exclusions, deployment identity | Clean commit `1f6ed51` passed the full release gate; protected Preview `dpl_GWf2jDsnsQuFTJMaahQ2aFtsrgBz` is Ready with 56 static assets, 41 functions, no internal-source leaks, and working nested health routes | Warning | Push the reviewed candidate, require a green remote CI run, then obtain explicit Production approval before promotion and bounded acceptance checks. |
