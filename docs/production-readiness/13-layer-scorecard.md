...
| 2 | APIs and backend logic | Input validation, auth, idempotency, safe errors, bounded dependencies; `api/`, `lib/` | Local tests pass across 41 routes. Exact candidate `307e48d` on protected Preview `dpl_4Ufb9xtj1AeLrfbsYL76Hn64sscH` returned liveness 200, readiness 503 fail-closed, app configuration 200, and concierge smoke 200. Production correctly denies protected unauthenticated APIs but is not the candidate artifact | Warning | Configure an isolated signed-beta environment, run safe signed-user Preview probes, and repeat complete bounded Production acceptance only after release approval. |
