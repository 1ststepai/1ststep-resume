...
- Hosted isolated-test path: a manual-only GitHub Actions workflow is implemented in `.github/workflows/isolated-database-verification.yml`. It uses a disposable local Supabase stack on a GitHub-hosted runner, has read-only repository permission, references no Production secret, performs no deployment, and destroys its source and restore databases after the run. It has not been pushed or executed, so it is implementation evidence only.
...
- [ ] Push the exact reviewed commit and manually run the hosted workflow; retain its content-free artifact and review every failure before changing a scorecard status.
