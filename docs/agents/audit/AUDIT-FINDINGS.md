...
| AUD-029 | HIGH | Static login page lacks security headers / Auth + Security | OPEN | BLOCKS signed beta | BLOCKS |
| AUD-030 | MEDIUM | Résumé sign-up CTA / Resume + admission truth | NEEDS REVERIFICATION | Conditional on signed admission/copy | BLOCKS truthful public claim review |
| AUD-031 | LOW | Temporary header-file provenance / Audit hygiene | NEEDS REVERIFICATION | No proven direct block | No proven direct block |

No Critical finding was proven. All 31 IDs remain visible and unresolved pending their specified evidence. AUD-018 is only partially mitigated: isolated RESUME-001 did not adopt the dirty WIP, but the stale release worktree remains. AUD-024's engineering-only assertion correction is not independent closure. A field marked UNKNOWN is not a pass.
...
## Cycle 3/4 findings — independent report ingested 2026-09-16

The independent Cycle 4 report (path/hash in `AUDIT-STATE.md`) restates its auditor's overwritten Cycle 3 findings. The other on-disk Cycle 3 report remains a separate input; filename collision is an audit-record integrity failure, not grounds to discard either finding. These IDs follow the auditor's assignments; Engineering's read-only checks below refine evidence without independently closing them.

- **AUD-029 (HIGH, OPEN; login security headers):** Production `GET /login.html` is 200 (1,026 bytes) from a static asset, with no CSP, `X-Frame-Options`, `X-Content-Type-Options`, `Referrer-Policy` or `Permissions-Policy`; its HSTS is only `max-age=63072000`. Control `GET /api/login-page` serves the same-sized page with the intended environment-specific CSP and those other headers; `/` also has its intended headers. Independently rechecked by Engineering on 2026-09-16; last-modified on the static page remains 2026-09-15 18:45:54 GMT. Root cause: `build-public-web.mjs` publishes `login.html` as a static file, which wins over the `vercel.json` rewrite to `api/login-page.js`, while the site-wide header rule excludes `login.html`. No account compromise is proven. Security/Auth and Release own a bounded candidate that either removes the static route collision or applies equivalent safe headers, with actual built-output and exact Preview GET tests for CSP/frame/nosniff/referrer/permissions plus Clerk initialization. **No Production mutation without a separate approval.** Blocks G1 sign-in shell, signed beta and Production release.
- **AUD-030 (MEDIUM, NEEDS REVERIFICATION; résumé CTA):** Three live `resume.1ststep.ai` “Create free account” links route to Production `/login.html?mode=sign-up&returnTo=%2Fapp`; the destination currently has AUD-029. The CTA text and target are proven, but misleading admission or inability to create a free résumé account is **not** proven: `betaMode=false` and a signed free user may legitimately use the résumé surface. Resume lane must verify signed admission and downstream entitlement/copy before deciding whether to adjust the CTA; do not remove or relabel it by inference. Public claim review remains blocked by AUD-029 and this unresolved admission distinction.
- **AUD-031 (LOW, NEEDS REVERIFICATION; temporary headers):** The auditor observed `audit_tmp_headers2.txt` and `audit_tmp_headers3.txt` in the integration worktree during Cycle 3; their writer removed them before inspection. Contents, cookie/token presence, exposure and exact authorship are UNKNOWN. This is not a proven credential leak. Use scratch-only, content-free header probes and avoid persisting auth/session headers; escalate only if recoverable evidence proves sensitive material. No direct release block is assigned on current evidence.

**Recurring findings:** AUD-003 remains OPEN because Production changed through a CLI/prebuilt path outside `main` and without a contemporaneous GitHub Production record. Read-only Vercel API metadata now declares `gitCommitSha=7412af1...` and the integration branch for app deployment `dpl_229cmvzCQvh3hvcXZuKBPf9EwCyy`, and all 88 pre-fix public allowlist assets match clean candidate bytes; however, the complete source tree at CLI build time and function artifact parity are not established. AUD-019 remains OPEN: the Product Owner's 2026-09-14 deferral allowed clean integration/Preview only, conditional on paid-access dependence before any beyond-Preview deployment. Production is now live, paid/owner reliance and actual entitlement behavior are UNKNOWN, and a new owner scope decision is required. Neither finding is downgraded by a successful public asset check or Clerk readiness flag.
