...
| AUD-005 + AUD-007 / RESUME-001 | Product Owner authority contract recorded in `AUDIT-DECISIONS.md` on 2026-09-13. Proposed Application Content & Truth subsystem overlaps uncommitted `client/resume-reconciliation.js`, `scripts/resume-reconciliation-test.mjs`, `concierge.js` in the PR #80 release worktree (`89cbd017`), and applicant-vault/package/session/concierge edits in the main product checkout (`7a57fd7`). The PR #80 Claude writer stopped at a usage limit; sole-writer transfer has not been confirmed. | Read-only `node scripts/resume-reconciliation-test.mjs` passes (8 named cases plus edge checks). Draft UI still selects browser text by default and passes `savedResumeText()` to package generation without base identity/version/hash in package input; this is not the approved target. No new source edit or test was made. | None. | OWNERSHIP HANDOFF PENDING — preserve both worktrees. Not READY FOR INDEPENDENT AUDIT; AUD-005/AUD-007 remain unresolved. |
