...
- **Cycle/acceptance:** Verify canonical repo/worktree/branch/SHA and clean/reconciled source; version/extension digest; exact-head CI/tests/security; Preview live path; current Production baseline and source parity; required signed runtime, database/RLS, capacity, recovery, support, and rollback evidence. Obtain the **current independent-audit status for this exact candidate**, distinguish Cycle 0's partially reviewed PR from full coverage, and reconcile every unresolved Critical/High and applicable gate-blocking finding with independent resolution evidence. Missing evidence stays unknown.
