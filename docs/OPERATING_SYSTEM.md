...
| `partners.1ststep.ai` | Partner-program explanation and entry to separate partner-role onboarding | `partners-landing/`; authenticated `/partner` and `/api/partner` ship with Vercel project `1ststep-resume` | The landing remains public. Partner applicants verify one Clerk identity and receive an opaque app session, then explicitly choose existing-user linking or affiliate-only access. Partner records are encrypted and isolated from applicant data; approval is administrator-only. | Existing users reuse their verified identity without a duplicate. Affiliate-only applicants receive no Job Agent profile, résumé, My Jobs data, or entitlement. |
...
| Partner role and referral attribution | Encrypted partner-role store and approved-only attribution endpoint; no payout ledger | The signed-in partner sees only their own role; administrator subjects review applications; attribution is first-write and self-referrals are rejected | Explicit role consent, opaque session, administrator review, and verified billing events only; beta referrals are never represented as earned commission |
