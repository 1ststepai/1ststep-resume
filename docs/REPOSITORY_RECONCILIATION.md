...
- Application-equivalent PR #80 Previews `dpl_3ZhHMrqY5sK69t3aJhXisAHpwDYn` at `1a8dfe5` and `dpl_5ka3ESoTPaxventdyh6bXSvNQt8F` at docs-only `9362ad1` are Ready, and all configured checks passed on both heads. Earlier authenticated Vercel curl verified clean `/concierge`, `/partner`, their JavaScript, and their CSS bytes match the candidate; direct HTML files differ only by Vercel's appended feedback script. Generic Preview scope contains 62 unique names and 13 of 126 release-required names, while the release branch has zero branch-specific overrides. PR #72 has five overrides but also lacks `JOB_AGENT_RECEIPT_SECRET`; copying it alone cannot close the Preview gate. Values were not inspected. Public `/api/app-config` on both observed deployments reports Clerk authentication disabled and restore access unavailable, proving the deployed authentication gate is closed without revealing any credential value.
