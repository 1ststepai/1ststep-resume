...
- PR #80 remains the sole consolidated app, partner-role, and extension v1.6 release path. Its release evidence includes merge commit `0f9dc3d`, which reconciles `origin/main`, plus implementation commit `4b344f5` for the conservative ATS capability matrix and one-action employer blocker contract. The branch is clean, pushed, mergeable, and its required GitHub/Vercel checks were green before this documentation-only update.
...
- Production and exact-head PR #80 Preview `https://1ststep-resume-j6zn0ak3b-1ststep.vercel.app` both return Clerk identity disabled. The exact Preview also returns restore access unavailable. All four branch-scoped Clerk names exist, while shared Preview Redis/Resend names also exist; therefore at least one branch override or supporting session/encryption value is empty or malformed. Authenticated persistence, partner-role access, extension-to-My-Jobs persistence, and final end-to-end Job Agent proof remain blocked until a new Preview returns `authentication.clerk.enabled: true` and completes a real sign-in flow.
...
- Three production aliases return HTTP 200 from Ready Vercel deployments. Vercel CLI metadata and public hashes now prove app matches clean baseline tree `8a6ba82`, resume matches `resume-tailor-landing/standalone/index.html` at `44a2f42` and PR #80, and partner root/CSS/JS match `partners-landing` at `7ec3acc`. PR #80 intentionally contains unreleased app and partner changes.
...
- Current production deployment IDs: app `dpl_DFvTeYBGbVcmoTGQZdCcegZTNysb`, resume `dpl_2UC7e3oFPiwLC4JJCz1mUV9r2DDb`, partners `dpl_3qctMKn1Cm7jT77gLGSGZwe9oZQr`. The app deployment restored the exact clean `origin/main` tree after a temporary EasyFunnel widget experiment was removed; live homepage and pricing bytes match the 82-file baseline build and make zero EasyFunnel requests. Resume metadata binds to `44a2f42`, and its root hash matches that commit and PR #80. Partner metadata binds to `7ec3acc`, and its root, CSS, and JavaScript hashes match that commit; PR #80 intentionally contains the unreleased replacement partner bundle.
- PR #80 exact-source Preview `dpl_5knK1Mh6MD2FP9WEndz6rZbhBfva` is Ready. Ten variables are branch-scoped, including all four expected Clerk names; however, after resetting `CLERK_IDENTITY_ENABLED` to exact lowercase `true`, `/api/app-config` still returns Clerk disabled, a null publishable key, and restore access available. At least one sensitive Clerk value is empty or malformed. Re-enter the development `pk_test_` publishable key, matching secret key, and complete PEM JWT public key, then redeploy before attempting OTP or authenticated persistence proof.
...
1. **PR #80 supersedes PR #72 and PR #79 as the single consolidated release candidate, but do not release it yet.** PR #72's exact v1.6.0 source and checksum are retained in PR #80 ancestry. PR #80 head `3044219` is clean and all configured GitHub checks are green, but production environment, authenticated persistence, and live-parity gates remain open. Merge, deployment, and Web Store publication remain gated.
...
4. **Keep deployment provenance controlled.** The overlapping CLI path is identified as `codex/fazier-badge-20260910`, and its current deployment returns Production to the exact clean-baseline tree. Coordinate that path during the release window, then verify the intended PR #80 commit and protected runtime evidence immediately before any approved promotion. Current production database/encryption/worker/tenant health remains unknown.
