...
- Application-equivalent PR #80 Previews `dpl_3ZhHMrqY5sK69t3aJhXisAHpwDYn` at `1a8dfe5`, `dpl_5ka3ESoTPaxventdyh6bXSvNQt8F` at docs-only `9362ad1`, and `dpl_GMJxTKNAhtr9BhnEftg9eqMsYwmV` at docs-only `119e4e0` are Ready; configured checks passed on each completed head. Earlier authenticated Vercel curl verified clean `/concierge`, `/partner`, their JavaScript, and their CSS bytes match the candidate; direct HTML files differ only by Vercel's appended feedback script. With owner authorization, six fresh sensitive non-provider values and the Clerk enable flag are now scoped only to PR #80; no value was printed. The remaining Preview inputs are the isolated Clerk development publishable key, secret key, and PEM JWT public key. The application now accepts `pk_test_` only outside Production and continues to require `pk_live_` in Production, matching Clerk's independent-Preview guidance. Redeploy and runtime proof remain pending until the three Clerk values are added.
