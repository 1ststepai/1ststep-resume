...
- Current production deployment IDs: app `dpl_2XSwQvgnCjhNWv5a3shXPDQRLbXe`, resume `dpl_2UC7e3oFPiwLC4JJCz1mUV9r2DDb`, partners `dpl_3qctMKn1Cm7jT77gLGSGZwe9oZQr`. The app deployment was manually created without Git SHA/ref metadata at 2026-09-10T14:25:28Z, after another manual deployment `dpl_AFReDSp4WKAuGCYjR1BDJtkE4ca8` eight minutes earlier. Public Job Agent asset hashes match preserved commit `7f401e8` and do not match PR #80 head `3044219`; production is therefore proven out of parity with the consolidated candidate. Resume and partner source parity remains unknown.
...
4. **Restore controlled deployment provenance before any release.** Current app production assets match predecessor `7f401e8`, not PR #80, and two manual no-Git-metadata deployments appeared during final verification. Identify and stop the overlapping deployment path, then verify the intended commit and protected runtime evidence immediately before any approved promotion. Current production database/encryption/worker/tenant health remains unknown.
