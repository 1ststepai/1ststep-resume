...
- Exact-head PR #80 Preview `dpl_3ZhHMrqY5sK69t3aJhXisAHpwDYn` is Ready at commit `1a8dfe5`, and all configured PR checks are green. Earlier authenticated Vercel curl verified clean `/concierge`, `/partner`, their JavaScript, and their CSS bytes match the candidate; direct HTML files differ only by Vercel's appended feedback script. Generic Preview scope contains 62 unique names and 13 of 126 release-required names, while the release branch has zero branch-specific overrides. PR #72 has five overrides but also lacks `JOB_AGENT_RECEIPT_SECRET`; copying it alone cannot close the Preview gate. Values were not inspected. Exact-head public `/api/app-config` reports Clerk authentication disabled and restore access unavailable, proving the deployed authentication gate is closed without revealing any credential value.
