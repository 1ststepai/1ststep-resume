...
- Current production deployment IDs: app `dpl_2XSwQvgnCjhNWv5a3shXPDQRLbXe`, resume `dpl_2UC7e3oFPiwLC4JJCz1mUV9r2DDb`, partners `dpl_3qctMKn1Cm7jT77gLGSGZwe9oZQr`. Vercel CLI metadata binds the app deployment to Codex actor commit `5b1ba79` (`Remove unavailable Fazier launch badge`) on `codex/fazier-badge-20260910`; predecessor `dpl_AFReDSp4WKAuGCYjR1BDJtkE4ca8` binds to `87eb60a` (`Add Fazier launch badge`). Those two commits add and then remove one line, leaving `5b1ba79` and clean baseline `8a6ba82` at identical tree `6f403f3b1e7936d97cf17765ecf4f1d5972e6678`. Resume metadata binds to `44a2f42`, and its root hash matches that commit and PR #80. Partner metadata binds to `7ec3acc`, and its root, CSS, and JavaScript hashes match that commit; PR #80 intentionally contains the unreleased replacement partner bundle.
- Application-equivalent PR #80 Previews `dpl_3ZhHMrqY5sK69t3aJhXisAHpwDYn` at `1a8dfe5`, `dpl_5ka3ESoTPaxventdyh6bXSvNQt8F` at docs-only `9362ad1`, and `dpl_GMJxTKNAhtr9BhnEftg9eqMsYwmV` at docs-only `119e4e0` are Ready; configured checks passed on each completed head. Earlier authenticated Vercel curl verified clean `/concierge`, `/partner`, their JavaScript, and their CSS bytes match the candidate; direct HTML files differ only by Vercel's appended feedback script. A branch-resolved, one-use environment pull corrected the earlier broad name count: PR #80 has zero overrides; generic Redis/rate/tier and email-delivery names resolve, but Vercel redacts sensitive values and their validity remains unknown. Clerk, encryption, audit, and receipt configuration are not available to this branch, and no `JOB_AGENT_RECEIPT_SECRET` entry was found. Temporary files were deleted and no secret value was printed. Public `/api/app-config` on all three observed deployments reports Clerk authentication disabled and restore access unavailable.
