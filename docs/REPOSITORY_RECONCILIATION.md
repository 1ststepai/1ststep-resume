...
- PR #80 evidence-only Preview `dpl_ENPNfyLyg57LUixMpKvhTTHUeFfd` is Ready/Staged at commit `442e228`. Authenticated Vercel curl verified clean `/concierge`, `/partner`, their JavaScript, and their CSS bytes match the candidate; direct HTML files differ only by Vercel's appended feedback script. Generic Preview scope contains 62 unique names and 13 of 126 release-required names, while the release branch has zero branch-specific overrides. PR #72 has five overrides but also lacks `JOB_AGENT_RECEIPT_SECRET`; copying it alone cannot close the Preview gate. Values were not inspected.
