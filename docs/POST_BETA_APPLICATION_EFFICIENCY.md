...
Keep Vault as the only candidate memory. Exact ordinary matches continue to auto-reuse. Atomic intent matching may **propose** a vault fact reference only when exactly one supported atomic intent is identified, polarity is unambiguous, the question is not compound or punctuated as bundled, unmatched semantic remainder is empty, scope is compatible, the source fact has allowlisted provenance, and the proposal remains review-only. Otherwise the result is UNKNOWN — NEEDS USER. The proposal never writes a new confirmed fact, never fills the employer form, never inverts a remembered answer, and never submits. User confirmation still creates an exact-question memory version through the existing remember/resolve path.

After one atomic intent is identified, only the matched intent text is removed. Any leftover non-stopword content — geography, nickname, suffix, school, certification, commute, notice period, or another field — fails closed. Commas, semicolons, colons, dashes, and slashes fail closed. `this <noun>` and equivalent contextual language is application-scoped. Equivalent-proposal provenance is an allowlist (`user answered application question`, explicit candidate confirmation, document-verified). Efficiency reason codes are a closed `unspecified` / `http_429` / `ats_*` vocabulary.
