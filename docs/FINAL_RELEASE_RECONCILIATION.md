...
| Preview environment audit | With owner authorization, six fresh non-provider values plus `CLERK_IDENTITY_ENABLED=true` are now scoped only to PR #80: encryption key/key ID, audit secret, receipt secret, rate-partition secret, and tier secret. Values were never printed. The remaining inputs are an isolated Clerk development publishable key, secret key, and PEM JWT public key; Preview code accepts `pk_test_`, while Production remains restricted to `pk_live_`. A redeploy and content-free runtime check remain required after those three values are added. |
...
2. In Clerk's development/Preview instance, copy the Publishable Key, Secret Key, and **Show JWT public key → PEM Public Key** into branch-scoped Vercel variables `CLERK_PUBLISHABLE_KEY`, `CLERK_SECRET_KEY`, and `CLERK_JWT_KEY`. Do not paste them into chat or a file. The six isolated non-provider secrets and enable flag are already provisioned for PR #80. Redeploy and re-run the content-free gate after the three Clerk values are present.
