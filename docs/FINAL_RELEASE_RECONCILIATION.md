...
| Vercel | Project/deployment metadata, aliases, status, and environment **names only** | Accessible. Runtime-bearing Preview deployment `dpl_E7h1ghMHF31tqAFFQN7Tfn1iF6M9` is Ready and GitHub deployment `6371383461` binds it to runtime/report commit `2be4174`. Its root, `/job-agent`, and `/partners.html` returned HTTP 200. Later evidence-only documentation commits do not change the recorded runtime hash, but their current checks still require GitHub verification. Secret values were neither requested nor printed. Exact Git SHA for the current production deployment remains unavailable. |
