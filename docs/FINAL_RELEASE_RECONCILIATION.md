...
| Preview environment-name audit | Generic Preview scope: 62 unique names, 13 of 126 required names present, 113 absent. PR #80 branch-specific scope: zero overrides. PR #72 branch-specific scope: five overrides, also without `JOB_AGENT_RECEIPT_SECRET`. No values were inspected. |
...
| Release Preview | Runtime-bearing `dpl_Awsa9tA7AwJpJbpDrVbsUNSiXzZP` is bound through GitHub deployment `6373675875` to implementation commit `76fd986`. Evidence-only `dpl_ENPNfyLyg57LUixMpKvhTTHUeFfd` is Ready/Staged at `442e228`; authenticated Vercel curl proved `/`, `/concierge`, and `/api/health/live` return 200, protected APIs fail closed, and candidate public asset hashes match after excluding Vercel's direct-HTML feedback-script injection. Authenticated Clerk/partner/My Jobs persistence is still untested. |
...
2. With owner approval, provision an isolated Preview secret set for PR #80 or map an approved existing complete set without printing values. Do not merely copy PR #72: its five overrides are incomplete and omit `JOB_AGENT_RECEIPT_SECRET`. Preserve PR #72's configuration until the replacement Preview passes and cleanup is separately approved.
