...
- The release branch has zero branch-specific Preview overrides. It inherits 62 unique generic Preview names, of which 13 of 126 release-required names are present. Six of seven durable-runtime requirements are named; `JOB_AGENT_RECEIPT_SECRET` is missing. Values were not inspected or validated. The exact Preview accepts its deployment URL and alias as origins and correctly returns `AUTH_REQUIRED` without authentication, so origin allowlisting is not the blocker; deliberate configuration mapping and authenticated runtime proof remain required.
...
| Preview environment-name audit | Generic Preview scope: 62 unique names, 13 of 126 required names present, 113 absent. Branch-specific scope: zero overrides. Durable-runtime names are 6 of 7, missing `JOB_AGENT_RECEIPT_SECRET`; no values were inspected. |
