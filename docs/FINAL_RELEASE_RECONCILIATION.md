...
| Vercel | Project/deployment metadata, aliases, status, environment **names only**, and public asset bytes | Accessible. Runtime-bearing Preview deployment `dpl_Awsa9tA7AwJpJbpDrVbsUNSiXzZP` is Ready and GitHub deployment `6373675875` binds it to implementation commit `76fd986`. The final documentation-only Preview is `dpl_4pJi6mHNgi6ProJBbvHRDqWw9hFN`, bound by GitHub deployment `6373832913` to `3044219`. Secret values were neither requested nor printed. The current Production deployment exposes no Git SHA/ref, but its public Job Agent asset bytes were matched to preserved commit `7f401e8`; it does not match PR #80. |
...
- During final verification, two manual Production deployments with no Git metadata replaced the prior app deployment: `dpl_AFReDSp4WKAuGCYjR1BDJtkE4ca8` at 2026-09-10T14:17:55Z and current `dpl_2XSwQvgnCjhNWv5a3shXPDQRLbXe` at 2026-09-10T14:25:28Z. Both were created by the `1ststepdotai` Vercel account; this release task did not create or promote them.
- Current `app.1ststep.ai` Job Agent assets are byte-identical to preserved commit `7f401e8` for `concierge.html` (`7e98d8af855bc955ddb922a5a33608abf45a743631f583b28a9a26b84afa6ec7`), `concierge.js` (`7d8a723f3cd76a35b00618366b34f487c045adccead13ad6521d1a1016dc7615`), and `persistent-concierge.css` (`f5ddd7450ef51cd4af42a5796b096fc825fbe55bec50ee0edb88f6db4579a1cc`). Live source parity with PR #80 head `3044219` therefore fails. `7f401e8` is a clean one-commit branch from `8a6ba82` whose trust work was integrated and then extended in PR #80; production is a partial predecessor, not the consolidated candidate.
...
| Public domains | HTTP 200 for app, resume, and partners. The live Job Agent safety-boundary probe passed (public routes 200 with CSP; private APIs 401), but live asset parity with PR #80 failed and authenticated behavior remains untested. |
...
| Runtime-bearing release Preview | Ready: `dpl_Awsa9tA7AwJpJbpDrVbsUNSiXzZP`, bound through GitHub deployment `6373675875` to `76fd986`, at `https://1ststep-resume-r7ebl7uyf-1ststep.vercel.app`. Final documentation-only Preview `dpl_4pJi6mHNgi6ProJBbvHRDqWw9hFN` is bound to head `3044219`. The Preview is browser-protected; authenticated Clerk/partner/My Jobs persistence is still untested. |
...
6. Stop uncoordinated manual Production deployments, identify the operator/worktree that created `dpl_AFRe...` and `dpl_2XSw...`, and retain `7f401e8` as preserved predecessor evidence. Do not roll back or promote anything while production differs from PR #80. Production deployment remains blocked until steps 2–5 pass and the exact approved candidate is re-verified immediately before promotion.
