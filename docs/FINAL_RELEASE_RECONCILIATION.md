...
- The release branch has zero branch-specific Preview overrides. It inherits 62 unique generic Preview names, of which 13 of 126 release-required names are present. Six of seven durable-runtime requirements are named; `JOB_AGENT_RECEIPT_SECRET` is missing. PR #72 has only five branch-specific overrides (`BETA_DATA_ENCRYPTION_KEY`, `BETA_DATA_ENCRYPTION_KEY_ID`, `JOB_AGENT_AUDIT_SECRET`, `RATE_LIMIT_HASH_SECRET`, and `TIER_SECRET`) and also lacks the receipt secret, so copying PR #72 alone cannot close the gate. Values were not inspected or validated. On exact-head Preview `dpl_3ZhHMrqY5sK69t3aJhXisAHpwDYn`, public `/api/app-config` returned `authentication.clerk.enabled: false`, a null publishable key, and `restoreAccessAvailable: false`. That endpoint exposes only booleans/public configuration; it did not expose credentials. The Preview accepts its deployment URL and alias as origins and correctly returns `AUTH_REQUIRED` without authentication, so origin allowlisting is not the blocker. At least one combined signed-session, encryption-keyring, tier-secret, or Clerk prerequisite is invalid or unavailable on the deployed runtime; deliberate isolated configuration and authenticated runtime proof remain required.
...
| Preview environment-name audit | Generic Preview scope: 62 unique names, 13 of 126 required names present, 113 absent. PR #80 branch-specific scope: zero overrides. PR #72 branch-specific scope: five overrides, also without `JOB_AGENT_RECEIPT_SECRET`. No values were inspected. Exact-head public runtime configuration reports Clerk authentication and restore access unavailable. |
...
| Release Preview | Exact-head `dpl_3ZhHMrqY5sK69t3aJhXisAHpwDYn` is Ready at `1a8dfe5`, and all configured PR checks are green. Earlier authenticated Vercel curl proved `/`, `/concierge`, and `/api/health/live` return 200, protected APIs fail closed, and candidate public asset hashes match after excluding Vercel's direct-HTML feedback-script injection. Exact-head `/api/app-config` reports Clerk disabled and restore access unavailable, so authenticated Clerk/partner/My Jobs persistence cannot yet be executed or claimed. |
...
2. With owner approval, provision an isolated Preview secret set for PR #80 or map an approved existing complete set without printing values, validate the existing Clerk Preview mapping without rotating or exposing its credentials, and redeploy. Do not merely copy PR #72: its five overrides are incomplete and omit `JOB_AGENT_RECEIPT_SECRET`. Preserve PR #72's configuration until the replacement Preview passes and cleanup is separately approved.
