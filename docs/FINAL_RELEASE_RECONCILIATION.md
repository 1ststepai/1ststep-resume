...
- PR #80 now has ten branch-specific Preview variables, including `CLERK_IDENTITY_ENABLED`, `CLERK_PUBLISHABLE_KEY`, `CLERK_SECRET_KEY`, and `CLERK_JWT_KEY` with the correct Preview/branch scope. After resetting the non-secret enable flag to exact lowercase `true`, exact-source redeploy `dpl_5knK1Mh6MD2FP9WEndz6rZbhBfva` still returned `authentication.clerk.enabled: false`, a null publishable key, and `restoreAccessAvailable: true`. Therefore at least one opaque Clerk value is empty or malformed; Vercel does not expose sensitive values for safe readback, and no value was printed or persisted. The application requires a `pk_test_` publishable key outside Production; actual JWT verification requires the complete PEM public key including its `BEGIN PUBLIC KEY` and `END PUBLIC KEY` lines.
...
| Preview environment audit | Four Clerk names and six non-provider values are scoped only to PR #80. Exact-head redeploy `dpl_5knK1Mh6MD2FP9WEndz6rZbhBfva` remains Ready, but its content-free `/api/app-config` result is `clerk.enabled: false`, null publishable key, and `restoreAccessAvailable: true` after the enable flag was reset to `true`. At least one sensitive Clerk value must be re-entered and then rechecked; no secret value was printed. |
...
2. Re-enter the branch-scoped Clerk values from the same Clerk development instance: `CLERK_PUBLISHABLE_KEY` must start with `pk_test_`, `CLERK_SECRET_KEY` must be that instance's secret key, and `CLERK_JWT_KEY` must contain the complete PEM public key including both `BEGIN PUBLIC KEY` and `END PUBLIC KEY` lines. Do not paste them into chat or a file. Redeploy and require `/api/app-config` to report Clerk enabled before sending an OTP.
