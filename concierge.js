...
let pendingExtensionCapture = null;
let extensionCaptureInFlight = false;
...
function renderJobAgentPolicyBundle(bundle = sessionCapabilities.jobAgentConsent?.policyBundle) {
  ...
  $('jobAgentConsentIntroduction').textContent = 'Check one box to continue. You can pause the agent and change this permission at any time.';
...
async function submitAgentAccess(event) {
  ...
  try {
    if (!agentRestoreChallenge) {
      const response = await fetchWithTimeout(`/api/subscription?action=restore-code&client=job-agent&email=${encodeURIComponent(email)}`, {}, REQUEST_TIMEOUTS.persistence);
      const data = await response.json().catch(() => ({}));
...
function extensionCaptureIdFromUrl() {
  return new URLSearchParams(window.location.search).get('jobCaptureId') || '';
}

function extensionCaptureText(value, limit) {
  return String(value || '').replace(/\s+/g, ' ').trim().slice(0, limit);
}

function validatedExtensionCapture(message) {
  const captureId = extensionCaptureText(message?.captureId, 100);
  const expectedCaptureId = extensionCaptureIdFromUrl();
  const job = message?.jobData && typeof message.jobData === 'object' ? message.jobData : {};
  const applyUrl = extensionCaptureText(job.applyUrl, 2_000);
  if (!captureId || captureId !== expectedCaptureId || !/^https?:\/\//i.test(applyUrl)) return null;
  const title = extensionCaptureText(job.jobTitle, 240);
  const employer = extensionCaptureText(job.company, 160);
  const jobDescription = String(job.jobDescription || '').trim().slice(0, 50_000);
  if (!title || !employer || jobDescription.length < 200) return null;
  return {
    captureId,
    role: {
      employer,
      title,
      requisitionId: extensionCaptureText(job.jobId, 160),
      jobDescription,
      directEmployerUrl: applyUrl,
      sourceUrl: applyUrl,
      sourceType: 'unknown',
      applyPathActive: false,
      remoteEligibility: extensionCaptureText(job.location, 240) || 'Captured from the posting; work location not verified',
      geographyEligibility: extensionCaptureText(job.location, 240) || 'Location not verified',
      salaryDisclosure: extensionCaptureText(job.salary, 160) || 'Compensation not verified',
      employmentType: extensionCaptureText(job.employmentType, 160) || 'Unknown',
      postedDate: extensionCaptureText(job.postedDate, 40),
      travel: 'Not verified',
      schedule: 'Not verified',
      sourceProvider: 'browser-capture',
      sourceEvidence: 'Captured from a page selected by the user; employer and apply path verification pending.',
    },
  };
}

function acknowledgeExtensionCapture(captureId) {
  window.postMessage({ type: '1STSTEP_JOB_CAPTURE_ACK', version: '1', captureId }, window.location.origin);
}

async function processPendingExtensionCapture() {
  if (!pendingExtensionCapture || extensionCaptureInFlight || !campaignSync.hydrated) return;
  if (!accountWorkflowIsAuthoritative()) {
    showDeskMessage('Sign in to Job Agent to save this captured job.', true);
    return;
  }
  extensionCaptureInFlight = true;
  const capture = pendingExtensionCapture;
  try {
    let result;
    const savedLocally = safeAction(() => {
      result = addRole(deskState, capture.role);
      deskState = result.state;
    });
    if (!savedLocally) return;
    clearTimeout(campaignSync.timer);
    await pushCampaignStore();
    if (campaignSync.status !== 'synced') {
      showDeskMessage('This job is ready, but secure saving needs attention. Keep this page open and try again.', true);
      return;
    }
    pendingExtensionCapture = null;
    acknowledgeExtensionCapture(capture.captureId);
    openJobs('Matches');
    const message = result?.duplicate ? 'This job is already in My Jobs.' : 'Job saved to My Jobs.';
    showDeskMessage(message);
    showToast(message);
  } finally {
    extensionCaptureInFlight = false;
  }
}

window.addEventListener('message', event => {
  if (event.source !== window || event.origin !== window.location.origin) return;
  if (event.data?.type !== '1STSTEP_JOB_CAPTURE' || event.data?.version !== '1') return;
  const capture = validatedExtensionCapture(event.data);
  if (!capture) {
    showDeskMessage('This page did not provide enough verified job information to save.', true);
    return;
  }
  pendingExtensionCapture = capture;
  processPendingExtensionCapture();
});
  ...
  $('answerMemoryForm').addEventListener('submit', async event => {
    ...
    const saveAnswer = async scope => {
      ...
      try {
        ...
        const savedVersion = saved.versions.find(version => version.version === saved.currentVersion) || saved.versions.at(-1);
        ...
        showToast(savedVersion?.autoReuse === true && savedVersion?.scope?.kind === 'candidate'
          ? 'Remembered for matching applications. Nothing submitted.'
          : 'Saved for this application only. Nothing submitted.');
      } catch (error) {
...
function start() {
  ...
  // Keep persisted learning and run controls reachable on the single-column
  // mobile workspace. The desktop disclosure remains collapsed by default.
  if (window.matchMedia('(max-width: 720px)').matches) $('agentProgress').open = true;
  renderAll();
...
loadSessionCapabilities().then(async () => {
  await hydrateAccountWorkflow();
  await processPendingExtensionCapture();
});
const extensionCaptureId = extensionCaptureIdFromUrl();
if (extensionCaptureId) {
  window.postMessage({ type: '1STSTEP_JOB_CAPTURE_REQUEST', version: '1', captureId: extensionCaptureId }, window.location.origin);
}
