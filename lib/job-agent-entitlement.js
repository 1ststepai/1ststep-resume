...
export function jobAgentAccessAllowed(auth, { env = process.env } = {}) {
  ...
  if (normalizeJobAgentEntitlements(auth?.entitlements).some(value => ACCESS_ENTITLEMENTS.has(value))) return true;
  const pilot = jobAgentPilotAccessForSubject(auth?.subject, env);
  return pilot.enforced === true && pilot.ok === true;
}
