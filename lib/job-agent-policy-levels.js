...
import { JOB_AGENT_POLICY_STATIC_DOCUMENTS } from './job-agent-policy-bundle.js';
...
/* Terms + Privacy versions AND the exact served document digests. Authorization
   version and counsel approval are NOT consulted: they govern acting on the user's
   behalf, not storing what the user typed. */
export function jobAgentDataPolicyConfiguration(env = process.env) {
  ...
  const termsDigest = String(JOB_AGENT_POLICY_STATIC_DOCUMENTS.terms.sha256 || '').trim().toLowerCase();
  const privacyDigest = String(JOB_AGENT_POLICY_STATIC_DOCUMENTS.privacy.sha256 || '').trim().toLowerCase();
  const ready = VERSION.test(termsVersion) && VERSION.test(privacyVersion) && SHA256.test(termsDigest) && SHA256.test(privacyDigest);
  return { ready, termsVersion, privacyVersion, termsDigest, privacyDigest };
}

/* The stored consent record must be active and bound to the CURRENT Terms and Privacy
   versions and the exact served document digests, with the internal scopes granted and
   all attestations affirmative. */
export function activeJobAgentDataConsent(existing, dataPolicy) {
  ...
  if (String(accepted.termsDigest || '').toLowerCase() !== dataPolicy.termsDigest
    || String(accepted.privacyDigest || '').toLowerCase() !== dataPolicy.privacyDigest) {
    return { ok: false, code: 'JOB_AGENT_DATA_CONSENT_RENEWAL_REQUIRED' };
  }
