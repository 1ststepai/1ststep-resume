...
export async function createCapturedDiscoveryRun({ config, subject, captureId, canonicalId = '', verifiedJob, now = new Date() }) {
  const created = await createJobAgentRun({
    ...config,
    subject,
    mission: { role: verifiedJob.title, target: 1 },
    taskType: 'direct_employer_discovery',
    idempotencyKey: canonicalId ? `canonical_${canonicalId}` : `capture_${captureId}`.slice(0, 128),
    now,
  });
