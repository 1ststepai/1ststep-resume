...
import { deleteAllCapturedJobs } from './captured-job-store.js';
import { deletePartnerAccount } from './partner-account.js';
...
const DEFAULT_OPERATIONS = Object.freeze({
  ...
  deleteCapturedJobs: deleteAllCapturedJobs,
  ...
  deletePartner: input => deletePartnerAccount({ ...input, secret: input.partitionSecret }),
...
export async function deleteTenantJobAgentOperationalData({ config, subject, operations = {} }) {
  ...
  const [runs, sessions, browserTasks, submissionTasks, receiptTasks, vault, learning, capturedJobs, campaign, consent, schedule, notifications, partner] = await Promise.all([
    ...
    execute.deleteCapturedJobs({ ...config, subject }),
    ...
    execute.deletePartner({ ...config, subject }),
  ]);
  ...
  return {
    ...
    vault, learning, capturedJobs, campaign, consent, schedule, notifications, partner, residualKeys,
  };
}
