...
function documentLines(value) {
  return String(value || '').split(/\r?\n/).map(line => cleanText(line, 60_000).replace(/\s+/g, ' ')).filter(Boolean);
}
function coverSourceLines(value) {
  const lines = documentLines(value);
  if (lines[0] === 'Dear Hiring Team,') lines.shift();
  if (lines.at(-1) === 'Sincerely,') lines.pop();
  if (lines.at(-1) === 'Thank you for considering my application.') lines.pop();
  return lines;
}
...
export function validateGeneratedApplicationPackage({ sourceResume, jobDescription, resumeText, coverLetterText, sourceMap = [] }) {
  ...
  // String links cannot prove semantic equivalence. Automatic artifacts require
  // the full source sequence, including headings and negation, to be preserved.
  // Tailoring remains a private reviewable draft; never certify rearranged facts.
  const sourceSequence = JSON.stringify(documentLines(sourceResume));
  if (JSON.stringify(documentLines(resume)) !== sourceSequence) issues.push('UNVERIFIED_RESUME_WORDING');
  if (coverLetter && JSON.stringify(coverSourceLines(coverLetter)) !== sourceSequence) issues.push('UNVERIFIED_COVER_LETTER_WORDING');
  ...
  for (const mapping of Array.isArray(sourceMap) ? sourceMap.slice(0, 80) : []) {
    ...
    if (claim && excerpt && !excerpt.includes(claim)) issues.push('UNSUPPORTED_SOURCE_MAPPING');
  }
  return {
...
export function parsePackageJson(raw) {
  ...
  if (!text) throw new Error('PACKAGE_RESPONSE_EMPTY');
  let parsed;
  try { parsed = JSON.parse(text); }
  catch { throw new Error('PACKAGE_RESPONSE_INVALID'); }
  ...
  if (typeof parsed.resume_text !== 'string' || typeof parsed.cover_letter_text !== 'string' || !Array.isArray(parsed.source_map) || !resumeText) throw new Error('PACKAGE_SCHEMA');
  return { resumeText, coverLetterText, sourceMap };
}

export function applicationPackageRequestFormat(request) {
  if (request.provider !== 'openai-compatible' || request.url !== 'https://api.openai.com/v1/responses') return request;
  const schema = {
    type: 'object', additionalProperties: false,
    required: ['resume_text', 'cover_letter_text', 'source_map'],
    properties: {
      resume_text: { type: 'string' }, cover_letter_text: { type: 'string' },
      source_map: { type: 'array', items: {
        type: 'object', additionalProperties: false, required: ['output_claim', 'source_excerpt'],
        properties: { output_claim: { type: 'string' }, source_excerpt: { type: 'string' } },
      } },
    },
  };
  return { ...request, body: { ...request.body,
    text: { format: { type: 'json_schema', name: 'application_package', strict: true, schema } },
    ...(request.body.reasoning ? { reasoning: { effort: 'none' } } : {}),
  } };
}

export function assertPackageResponseComplete(provider, payload) {
  if (payload?.status === 'incomplete' || payload?.stop_reason === 'max_tokens' || payload?.choices?.[0]?.finish_reason === 'length') throw new Error('PACKAGE_RESPONSE_TRUNCATED');
  if (payload?.status === 'failed') throw new Error('AI_PROVIDER_TRANSIENT');
  if (payload?.output?.some(item => item?.content?.some(block => block.type === 'refusal')) || payload?.choices?.[0]?.message?.refusal) throw new Error('PACKAGE_RESPONSE_REFUSED');
}

async function generatePackage({ mission, env, redis, runId, attempt, fetchImpl = fetch, now = new Date() }) {
  const request = applicationPackageRequestFormat(buildAiRequest({
    env: { ...env, AI_OPENAI_REASONING_EFFORT: 'none' }, quality: 'quality', task: 'application-package', system: SYSTEM_PROMPT, maxTokens: 3000,
...
- Reorder and rephrase for the target role, but omit every unsupported requirement. Tailored text remains a private draft requiring source review; source links alone cannot certify that its meaning is unchanged.
...
- Cover letter: specific and natural, only supported examples, no generic enthusiasm claims. Do not pad a short source to a word target. New or selected wording requires review before document files can be produced.
- Omit AI language unless the employer explicitly requests practical AI work.
- Keep the complete JSON compact enough for 3000 output tokens: prioritize concise source-supported text and short exact source mappings. Preserve line breaks between source lines. Omit lower-priority claims rather than inventing facts or leaving claims unsupported. Return all three fields and close the JSON object.
...
<verified_applicant_facts trust="candidate-confirmed-evidence-only">
${JSON.stringify(mission.verifiedFacts || [])}
</verified_applicant_facts>` }],
  }));
  ...
  try {
    ...
    assertPackageResponseComplete(request.provider, payload);
...
export async function executeClaimedApplicationPackageRun({ claimed, redis, dataEncryptionKey, env = process.env, objectStorage = jobAgentObjectStorageConfiguration(env), fetchImpl = fetch, now = new Date(), authorizationCheck = null }) {
  ...
  try {
    if (!await heartbeatJobAgentRun({ redis, runId: run.id, leaseToken, dataEncryptionKey, now, lifecycleState: 'Preparing' })) return null;
    ...
    if (!await heartbeatJobAgentRun({ redis, runId: run.id, leaseToken, dataEncryptionKey, now: clock(), lifecycleState: 'Preparing' })) return null;
    const sourceResume = [run.mission.resumeText, ...(run.mission.verifiedFacts || []).map(fact => fact.value)].join('\n');
    const textQa = validateGeneratedApplicationPackage({ sourceResume, jobDescription: run.mission.jobDescription, ...generated });
    ...
    const result = {
      ...
      baseResume: run.mission.baseResume, verifiedFactsHash: run.mission.verifiedFactsHash,
      ...
      generatedAt: clock().toISOString(), provider: generated.provider, model: generated.model,
    ...
    if (authorizationCheck) await authorizationCheck();
    if (qa.issues.length) return waitForUserJobAgentRun({ redis, runId: run.id, leaseToken, dataEncryptionKey, result, reasonCode: 'PACKAGE_REVIEW_REQUIRED', now: clock() });
    const finished = await finishJobAgentRun({ redis, runId: run.id, leaseToken, dataEncryptionKey, result, now: clock() });
    ...
    return failJobAgentRun({ redis, runId: run.id, leaseToken, dataEncryptionKey, errorCode: code, retryable, now: clock() });
  }
}
