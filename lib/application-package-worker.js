...
async function generatePackage({ mission, env, redis, runId, attempt, fetchImpl = fetch, now = new Date() }) {
  const request = buildAiRequest({
    env, quality: 'quality', system: SYSTEM_PROMPT, maxTokens: 3000,
...
<verified_direct_employer_role employer="${mission.employer}" requisition="${mission.requisitionId}" title="${mission.title}">
${mission.jobDescription}
</verified_direct_employer_role>
