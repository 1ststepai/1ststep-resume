...
export function applyEmployerInspectionPlan(session, plan, now = new Date()) {
  ...
  if (plan.status === 'waiting-for-user') {
    for (const item of plan.actions) {
      updated = addApplicationAction(updated, { type: item.type, summary: item.summary, metadata: { fieldKey: item.fieldKey, fieldRef: item.fieldRef, question: item.question || null, riskCategory: item.riskCategory || null, canSkipJob: item.canSkipJob === true, employer: item.employer, role: item.role, requisition: item.requisition, currentStep: item.currentStep, exactBlocker: item.exactBlocker, why: item.why, suggestedAnswer: item.suggestedAnswer, sourceFact: item.sourceFact, primaryAction: item.primaryAction } }, now);
    }
