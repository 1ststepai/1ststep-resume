...
export function reconcilePackageResumeInput({ vault, browserText = '' } = {}) {
  ...
  const conflicts = [];
  if (!selected) {
    if (legacy.length < 200) throw new Error('A reviewed base resume is required.');
    conflicts.push(...conflictsWithFacts(legacy, 'legacy-browser', facts));
    return {
      status: conflicts.length ? 'needs-review' : 'legacy-compatibility',
      conflicts, resumeText: legacy, verifiedFacts: facts, verifiedFactsHash: factsHash,
      baseResume: { documentId: 'legacy-browser:' + digest(legacy), version: 1, sha256: digest(legacy),
        source: 'legacy-browser', provenance: '1ststep_resume', reviewState: 'legacy-unverified' },
    };
  }
