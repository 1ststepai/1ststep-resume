...
const SAFE_REASON_CODE = /^[A-Za-z][A-Za-z0-9_-]{0,79}$/;
...
const POLARITY_BLOCK = /\b(?:not|never|no longer|unable|without|lack|lacks|lacking|lost|denied|expired|cannot)\b|\b(?:un|in)(?:authorized|eligible)\b|\b(?:have no|has no|had no|with no|no need|no requirement)\b/;
const ADDITIONAL_REQUEST = /\b(?:also|additionally|including|as well|along with|together with|in addition)\b|\b(?:provide|list|describe|explain|attach|upload|specify) (?:your|the|a|an)\b/;
const IMPLICIT_EMPLOYER_SCOPE = /\bour\b|\bthis (?:company|employer|role|position|job|office|opening|requisition|team|site)\b|\bheadquarters\b|\b(?:join|with|for) us\b/;
const INFERRED_PROVENANCE = /infer|generat|derived|model|heuristic|estimated|predict/;

export const ATOMIC_ANSWER_INTENTS = Object.freeze([
  { id: 'work_authorization', confirmation: 'always', allowsPreference: false, fieldKeys: ['authorization'], patterns: [/authorized to work/, /work authorization/, /legally (?:authorized|eligible) to work/, /right to work/, /eligible to work in/] },
  { id: 'sponsorship', confirmation: 'always', allowsPreference: false, fieldKeys: ['sponsorship'], patterns: [/visa sponsor/, /require sponsorship/, /need sponsorship/, /h-?1b/, /immigration sponsorship/] },
  ...
  { id: 'start_availability', confirmation: 'review', allowsPreference: false, fieldKeys: ['availability', 'startDate'], patterns: [/start date/, /available to start/, /when can you start/, /earliest start/] },
  { id: 'legal_first_name', confirmation: 'review', allowsPreference: false, fieldKeys: ['firstName'], patterns: [/legal first name/, /\byour first name\b/, /^first name$/] },
  { id: 'legal_last_name', confirmation: 'review', allowsPreference: false, fieldKeys: ['lastName'], patterns: [/legal last name/, /\byour last name\b/, /^last name$/, /family name/] },
  { id: 'email', confirmation: 'review', allowsPreference: false, fieldKeys: ['email'], patterns: [/email address/, /\byour email\b/, /^email$/] },
  { id: 'phone', confirmation: 'review', allowsPreference: false, fieldKeys: ['phone'], patterns: [/phone number/, /mobile number/, /\byour phone\b/, /^phone$/] },
...
export function isCompoundQuestion(question) {
  ...
  if (/\band\/or\b/.test(masked) || /\b[a-z]{4,}\/[a-z]{4,}\b/.test(masked)) return true;
...
export function hasImplicitEmployerScope(question) {
  ...
  return IMPLICIT_EMPLOYER_SCOPE.test(normalized);
}

function hasUnresolvedAdditionalRequest(normalized, intent) {
  if (ADDITIONAL_REQUEST.test(maskSingleIntentConnectives(normalized))) return true;
  let remainder = maskSingleIntentConnectives(normalized);
  for (const pattern of intent.patterns) remainder = remainder.replace(pattern, ' ');
  remainder = remainder.replace(/[?.,]/g, ' ').replace(/\s+/g, ' ').trim();
  return ADDITIONAL_REQUEST.test(remainder);
}
...
function contentFreeViolationPath(value, path) {
  ...
  if (/\s/.test(value) && /[A-Z]/.test(value.slice(1))) contentFreeViolation(path);
  if (/\s/.test(value)) {
...
function contentFreeReasonCode(value) {
  ...
  if (!SAFE_REASON_CODE.test(reasonCode)) throw new Error('ATS failure reason codes in efficiency records must be content-free.');
  const digitCount = (reasonCode.match(/\d/g) || []).length;
  if (digitCount > 4) throw new Error('ATS failure reason codes in efficiency records must be content-free.');
...
export function classifyCanonicalIntent(question) {
  ...
  if (isCompoundQuestion(normalized)) return null;
  const matches = ATOMIC_ANSWER_INTENTS.filter(intent => intent.patterns.some(pattern => pattern.test(normalized)));
  if (matches.length !== 1) return null;
  if (hasUnresolvedAdditionalRequest(normalized, matches[0])) return null;
  return matches[0];
}

function provenanceIsInferred(version = {}) {
  const provenance = text(version.provenance || version.originalSource || version.source, 160).toLowerCase();
  return version.inferred === true || INFERRED_PROVENANCE.test(provenance);
}
...
export function classifyLearningState(input = {}) {
  ...
  const inferred = INFERRED_PROVENANCE.test(provenance) || input.inferred === true;
...
function sourceClassEligible(intent, version) {
  if (provenanceIsInferred(version)) return false;
