...
const RESTORE_NONCE = /^[a-f0-9]{24}$/;
const RESTORE_CLIENTS = new Set(['legacy', 'job-agent']);
...
const CONSUME_RESTORE_SCRIPT = `
if redis.call('DEL', KEYS[1]) == 1 then return 'consumed' end
return 'missing'
`;

function restoreChallengeKey({ subject, nonce, client, partitionSecret }) {
  const normalizedSubject = safeSubject(subject);
  const normalizedNonce = String(nonce || '');
  const normalizedClient = String(client || '');
  if (!RESTORE_NONCE.test(normalizedNonce) || !RESTORE_CLIENTS.has(normalizedClient) || String(partitionSecret || '').length < 32) {
    throw new Error('A valid restore challenge is required.');
  }
  const id = createHmac('sha256', partitionSecret)
    .update(`subscription-restore.v1|${normalizedSubject}|${normalizedClient}|${normalizedNonce}`)
    .digest('hex');
  return `1ststep:subscription-restore:v1:${id}`;
}
...
export function restoreChallengeRuntimeConfiguration(env = process.env) {
  const url = String(env.UPSTASH_REDIS_REST_URL || '');
  const token = String(env.UPSTASH_REDIS_REST_TOKEN || '');
  const partitionSecret = String(env.RATE_LIMIT_HASH_SECRET || env.TIER_SECRET || '');
  if (!url || !token || partitionSecret.length < 32) return null;
  const cacheKey = `${url}|${token.slice(-8)}`;
  if (!redisClients.has(cacheKey)) redisClients.set(cacheKey, new Redis({ url, token }));
  return { redis: redisClients.get(cacheKey), partitionSecret };
}
...
export async function registerRestoreChallenge({ redis, subject, nonce, client, partitionSecret, ttlSeconds }) {
  const ttl = Math.max(1, Math.min(600, Math.floor(Number(ttlSeconds) || 0)));
  const key = restoreChallengeKey({ subject, nonce, client, partitionSecret });
  const result = await redis.set(key, 'pending', { nx: true, ex: ttl });
  if (result !== 'OK') throw new Error('Could not register a unique restore challenge.');
  return { registered: true };
}

export async function consumeRestoreChallenge({ redis, subject, nonce, client, partitionSecret }) {
  const key = restoreChallengeKey({ subject, nonce, client, partitionSecret });
  const result = await redis.eval(CONSUME_RESTORE_SCRIPT, [key], []);
  return { consumed: result === 'consumed' || Number(result) === 1 };
}

export async function invalidateRestoreChallenge({ redis, subject, nonce, client, partitionSecret }) {
  const key = restoreChallengeKey({ subject, nonce, client, partitionSecret });
  await redis.del(key);
...
export const USER_SESSION_RESTORE_SCRIPTS = Object.freeze({ CONSUME_RESTORE_SCRIPT });
