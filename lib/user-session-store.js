...
function safeAccountId(value, { optional = true } = {}) {
  const accountId = String(value || '');
  if (!accountId && optional) return '';
  if (!ACCOUNT_ID.test(accountId)) throw new Error('A valid immutable account ID is required.');
  return accountId;
}
...
export async function createUserSession({ redis, subject, accountId = '', tier, entitlements = [], partitionSecret, dataEncryptionKey, now = new Date(), ttlSeconds = SESSION_TTL_SECONDS, maxSessions = MAX_SESSIONS_PER_SUBJECT }) {
  ...
  const record = JSON.stringify({
    version: 1,
    envelope: encryptSession({ version: 3, subject: normalizedSubject, accountId: normalizedAccountId || undefined, tier: normalizedTier, entitlements: normalizedEntitlements, createdAt: now.toISOString(), expiresAt }, { dataEncryptionKey, aad: key }),
  });
  ...
  return { token, subject: normalizedSubject, accountId: normalizedAccountId || undefined, tier: normalizedTier, entitlements: normalizedEntitlements, createdAt: now.toISOString(), expiresAt, maxAgeSeconds: ttl };
}
...
export async function readUserSession({ redis, token, partitionSecret, dataEncryptionKey, now = new Date() }) {
  ...
  try {
    ...
    const accountId = safeAccountId(session.accountId);
    const tier = safeTier(session.tier);
    const entitlements = safeEntitlements(session.entitlements);
    const expiresAt = new Date(session.expiresAt);
    if (![1, 2, 3].includes(session.version) || !Number.isFinite(expiresAt.getTime()) || expiresAt <= now) {
      await redis.eval(DELETE_SCRIPT, [key, subjectIndexKey(subject, partitionSecret)], []).catch(() => false);
      return null;
    }
    return { subject, accountId: accountId || undefined, tier, entitlements, createdAt: session.createdAt, expiresAt: expiresAt.getTime(), sessionToken: token, authentication: 'opaque-session' };
  } catch {
    return null;
  }
}
