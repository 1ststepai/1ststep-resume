...
export async function probePostgresTenantStore({ configuration = postgresTenantStoreConfiguration() } = {}) {
  if (!configuration.enabled) return { status: 'disabled' };
  if (!configuration.ready) return { status: 'unavailable' };
  try {
    const tenantId = '0'.repeat(40);
    const sql = configuration.getSql();
    const results = await sql.transaction([
      sql`select set_config('app.tenant_id', ${tenantId}, true)`,
      sql`set local role job_agent_backend`,
      sql`select
          current_user = 'job_agent_backend' as role_ok,
          current_setting('app.tenant_id', true) = ${tenantId} as tenant_context_ok,
          to_regclass('public.app_tenants') is not null as schema_ok`,
    ], { isolationLevel: 'ReadCommitted', readOnly: true });
    const row = Array.isArray(results?.[2]) ? results[2][0] : null;
    return { status: row?.role_ok === true && row?.tenant_context_ok === true && row?.schema_ok === true ? 'healthy' : 'unavailable' };
  } catch {
