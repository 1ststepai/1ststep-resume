...
export async function refreshTenantJobCardFreshness({
  ...
  const selected = cards.filter(card => ACTIVE_CARD_STATUSES.has(card.status) && ['greenhouse', 'lever', 'ashby'].includes(String(card.sourceProvider || '').toLowerCase())).slice(0, Math.max(1, Math.min(10, Number(maxCards) || 5)));
