...
function normalizedRoleEvidence(value = {}) {
  return {
    location: text(value.location, 180), remote: value.remote === true,
    workplaceType: text(value.workplaceType, 80), employmentType: text(value.employmentType, 80),
    salaryMin: Number(value.salaryMin) > 0 ? Math.round(Number(value.salaryMin)) : null,
    salaryMax: Number(value.salaryMax) > 0 ? Math.round(Number(value.salaryMax)) : null,
    salaryCurrency: text(value.salaryCurrency || 'USD', 3).toUpperCase(),
    salaryDisclosure: text(value.salaryDisclosure, 300),
    compensationBasis: ['base', 'ote', 'total compensation', 'unknown'].includes(value.compensationBasis) ? value.compensationBasis : 'unknown',
    travelDisclosure: text(value.travelDisclosure, 180),
    peopleLeadershipRequired: value.peopleLeadershipRequired === true,
    categoryManagementRequired: value.categoryManagementRequired === true,
    verifiedAt: text(value.verifiedAt, 40),
  };
}
...
function transmissionApprovalScope(session) {
  ...
  return {
    ...
    directEmployerUrl: directUrl(session?.role?.directEmployerUrl), roleEvidence: normalizedRoleEvidence(session?.role?.evidence),
    documentVersion: text(session?.documentVersion, 180), fields,
  };
}
...
export function createApplicationSession(input = {}, now = new Date()) {
  ...
  const role = { employer: text(input.employer, 160), title: text(input.title, 200), requisitionId: text(input.requisitionId, 160), directEmployerUrl, evidence: normalizedRoleEvidence(input.roleEvidence) };
  const evidenceMapSummary = {
    mappedClaims: Math.max(0, Math.min(500, Math.round(Number(input.evidenceMapSummary?.mappedClaims) || 0))),
    qaIssues: Array.isArray(input.evidenceMapSummary?.qaIssues) ? input.evidenceMapSummary.qaIssues.slice(0, 30).map(item => text(item, 100)).filter(Boolean) : [],
  };
...
export function applicationSessionPublicSummary(session) {
  return {
    ...
    proposedFields: session.proposedFields, evidenceMapSummary: session.evidenceMapSummary || { mappedClaims: 0, qaIssues: [] }, formCheckpoint: session.formCheckpoint, approvals: session.approvals, workerExecution: session.workerExecution || null,
