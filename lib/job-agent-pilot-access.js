import { isProductOwnerSubject } from './admin-subject.js';
...
export function jobAgentPilotConfiguration(env = process.env) {
  ...
  const tenantIds = [...new Set(String(env.JOB_AGENT_PILOT_ALLOWED_TENANTS || '').split(/[,;\s]+/).map(value => value.trim().toLowerCase()).filter(Boolean))];
...
export function jobAgentPilotAccessForSubject(subject, env = process.env) {
  const configuration = jobAgentPilotConfiguration(env);
  if (!configuration.enforced) return { ok: true, enforced: false, code: null, maxUsers: null, invitedTenantCount: 0 };
  if (isProductOwnerSubject(subject)) {
    return { ok: true, enforced: true, status: 200, code: null, maxUsers: configuration.maxUsers, invitedTenantCount: configuration.invitedTenantCount };
  }
...
export function publicJobAgentPilotAccess(result = {}) {
  ...
  const tenantId = String(result.tenantId || '').toLowerCase();
  if (/^[a-f0-9]{40}$/.test(tenantId)) publicResult.admissionHint = tenantId.slice(0, 8);
  return publicResult;
}
