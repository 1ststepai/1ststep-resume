...
const WORKER_HEARTBEAT_KEY = `${BASE}:background-worker-heartbeat`;
const WORKER_HEARTBEAT_TTL_SECONDS = 3 * 24 * 60 * 60;
...
export async function recordJobAgentWorkerHeartbeat({ redis, now = new Date(), outcome = 'started' } = {}) {
  if (!redis) return { recorded: false };
  if (!['started', 'succeeded', 'failed'].includes(outcome)) throw new Error('Unsupported Job Agent worker heartbeat outcome.');
  const heartbeat = { schemaVersion: 1, contentFree: true, lastSeenAt: now.toISOString(), outcome };
  await redis.set(WORKER_HEARTBEAT_KEY, JSON.stringify(heartbeat), { ex: WORKER_HEARTBEAT_TTL_SECONDS });
  return { recorded: true, heartbeat };
}

function workerHealth(raw, now) {
  if (!raw) return { status: 'unknown', lastSeenAt: null, outcome: null, ageSeconds: null };
  try {
    const heartbeat = typeof raw === 'string' ? JSON.parse(raw) : raw;
    const seen = new Date(heartbeat.lastSeenAt);
    const ageMs = now.getTime() - seen.getTime();
    if (!Number.isFinite(ageMs) || ageMs < 0 || !['started', 'succeeded', 'failed'].includes(heartbeat.outcome)) throw new Error('invalid');
    const status = ageMs > WORKER_STALE_AFTER_MS
      ? 'stale'
      : heartbeat.outcome === 'failed'
        ? 'failed'
        : heartbeat.outcome === 'started' && ageMs > WORKER_INCOMPLETE_AFTER_MS
          ? 'incomplete'
          : heartbeat.outcome === 'started' ? 'running' : 'healthy';
    return { status, lastSeenAt: seen.toISOString(), outcome: heartbeat.outcome, ageSeconds: Math.floor(ageMs / 1000) };
  } catch {
    return { status: 'unknown', lastSeenAt: null, outcome: null, ageSeconds: null };
  }
}

export async function readJobAgentWorkerHealth({ redis, now = new Date() } = {}) {
  if (!redis) return { status: 'unknown', lastSeenAt: null, outcome: null, ageSeconds: null };
  return workerHealth(await redis.get(WORKER_HEARTBEAT_KEY), now);
}
...
export async function readJobAgentOperationalMetrics({ redis, days = 2, now = new Date() } = {}) {
  ...
  const backgroundWorker = await readJobAgentWorkerHealth({ redis, now });
  return {
