...
import {
  START_AVAILABILITY_MAX_AGE_MS, attachEquivalentAnswerProposals, classifyCanonicalIntent,
  hasImplicitEmployerScope, memoryVersionIsReusable,
} from './application-efficiency-foundation.js';
...
export function matchingAnswerMemory(vault, { question, applicationId, employer, now = Date.now() }) {
  ...
  return [...(vault.facts || [])].sort((a,b) => priority(a) - priority(b)).find(fact => {
    ...
    return fact.status === 'active' && s?.memory === true && normalizeMemoryQuestion(s.question) === normalizeMemoryQuestion(question)
      && memoryVersionIsReusable(v, { question, now })
...
export function rememberApplicationAnswer(vault, session, input, now = new Date()) {
  ...
  const implicitEmployer = hasImplicitEmployerScope(question) || hasImplicitEmployerScope(statement);
  const kind = isPermission || isSensitive || namedEmployer || implicitEmployer ? 'application' : ['application', 'employer'].includes(input.scope) ? input.scope : 'candidate';
  ...
  const expiresAt = input.expiresAt
    ...
    : intent?.id === 'start_availability'
      ? new Date(now.getTime() + START_AVAILABILITY_MAX_AGE_MS).toISOString()
      : null;
...
// Called only after the server checks active candidate authorization. Reuse the
// current private reference; employer filling and submission keep their own gates.
export function reuseApplicationAnswers(session, vault, now = new Date()) {
  if (session.state === 'Paused' || session.submissionAttempt || vault?.consent?.status !== 'granted'
      || !vault.consent.scopes?.includes('confirmed-facts')) return session;
  let updated = session;
  for (const action of session.actions || []) {
    if (action.status !== 'open' || action.type !== 'AMBIGUOUS_FACT') continue;
    const question = String(action.metadata?.question || '').trim();
    if (!question || secret.test(question) || sensitive.test(question) || permission.test(question)) continue;
    const fact = matchingAnswerMemory(vault, { question, applicationId: session.id, employer: session.role.employer, now: now.getTime() });
    const version = fact?.versions?.find(item => item.version === fact.currentVersion);
    if (!version || version.autoReuse !== true || version.sensitivity !== 'standard'
        || version.verificationState !== 'user-confirmed' || version.scope.category === 'permission') continue;
    updated = resolveApplicationAnswer(updated, vault, { actionId: action.id, factId: fact.id, factVersion: fact.currentVersion, confirmed: true }, now);
  }
  return attachEquivalentAnswerProposals(updated, vault, now);
}
