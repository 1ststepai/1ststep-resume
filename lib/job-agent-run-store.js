...
import { CREATE_UNIQUE_PACKAGE_SCRIPT, PACKAGE_HISTORY_REQUIRED, packageIdentityAliases, packageIdentityMatch, reconcilePackageHistory } from './application-package-identity.js';
...
async function createJobAgentRunForResolvedTenant({ redis, tenantId, dataEncryptionKey, mission, taskType = 'direct_employer_discovery', idempotencyKey: idem, now = new Date() }) {
  ...
  const uniquePackage = privatePayload && !safeMission.revision;
  const historicalRunId = uniquePackage ? await reconcilePackageHistory({ redis, tenantId, dataEncryptionKey, mission: safeMission, readRun: readJobAgentRunForTenant }).catch(() => { throw new Error(PACKAGE_HISTORY_REQUIRED); }) : '';
  ...
  const keys = [key, idempotencyKey(tenantId, idem), dueKey(), tenantIndexKey(tenantId)];
  const args = [JSON.stringify(record), id, String(now.getTime()), String(RUN_TTL_SECONDS), String(IDEMPOTENCY_TTL_SECONDS)];
  if (uniquePackage) {
    keys.push(`${BASE}:tenant:${tenantId}:package-identities`);
    args.push(JSON.stringify(packageIdentityAliases(safeMission)), historicalRunId, String(365 * 24 * 60 * 60));
  }
  const result = await redis.eval(uniquePackage ? CREATE_UNIQUE_PACKAGE_SCRIPT : CREATE_SCRIPT, keys, args);
  const [status, returnedId] = Array.isArray(result) ? result : ['error', ''];
  if (status === 'history_required') throw new Error(PACKAGE_HISTORY_REQUIRED);
  ...
  if (!stored || stored.tenantId !== tenantId) throw new Error(uniquePackage ? PACKAGE_HISTORY_REQUIRED : 'Job Agent run could not be restored.');
  if (uniquePackage && (stored.taskType !== 'application_package' || packageIdentityMatch(safeMission, publicRun(stored, { dataEncryptionKey }).mission) !== 'exact')) throw new Error(PACKAGE_HISTORY_REQUIRED);
...
export async function deleteAllJobAgentRuns({ redis, subject, partitionSecret: secret }) {
  ...
  await redis.del(`${BASE}:tenant:${tenantId}:package-identities`);
  return { deleted };
}
