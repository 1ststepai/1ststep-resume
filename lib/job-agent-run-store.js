...
if tonumber(ARGV[9]) > serverNow or tonumber(ARGV[10]) > serverNow then return {'not_due'} end
if record.attempt >= record.maxAttempts then
  record.status = 'Failed'
  record.lifecycleState = 'Failed Safely'
  record.version = record.version + 1
  record.updatedAt = ARGV[4]
  record.leaseUntil = ''
  record.leaseTokenHash = ''
  record.lastErrorCode = 'WORKER_RECOVERY_EXHAUSTED'
  record.events = record.events or {}
  local event = cjson.decode(ARGV[8])
  event.type = 'RUN_FAILED_SAFELY'
  event.state = 'Failed Safely'
  event.attempt = record.attempt
  event.errorCode = record.lastErrorCode
...
if not record.leaseUntil or record.leaseUntil == '' or record.leaseUntil <= ARGV[4] then return {'lease_lost'} end
local serverTime = redis.call('TIME')
local serverNow = tonumber(serverTime[1]) * 1000 + math.floor(tonumber(serverTime[2]) / 1000)
if tonumber(ARGV[17]) <= serverNow then return {'lease_lost'} end
record.version = record.version + 1
...
export function validateApplicationPackageInput(input = {}) {
  ...
  const evidenceInput = input.roleEvidence && typeof input.roleEvidence === 'object' ? input.roleEvidence : {};
  const baseInput = input.baseResume && typeof input.baseResume === 'object' ? input.baseResume : null;
  const safe = {
    ...
    baseResume: baseInput ? {
      documentId: String(baseInput.documentId || '').trim().slice(0, 128), version: Number(baseInput.version),
      sha256: String(baseInput.sha256 || '').trim(), source: String(baseInput.source || '').trim().slice(0, 80),
      provenance: String(baseInput.provenance || '').trim().slice(0, 160),
      reviewState: String(baseInput.reviewState || '').trim().slice(0, 40),
    } : null,
    verifiedFacts: Array.isArray(input.verifiedFacts) ? input.verifiedFacts.slice(0, 100).map(fact => ({
      id: String(fact.id || '').trim().slice(0, 128), fieldKey: String(fact.fieldKey || '').trim().slice(0, 64),
      version: Number(fact.version), value: String(fact.value || '').trim().slice(0, 12_000),
      provenance: String(fact.provenance || '').trim().slice(0, 160),
      verificationState: String(fact.verificationState || '').trim().slice(0, 32),
    })) : [],
    verifiedFactsHash: String(input.verifiedFactsHash || '').trim(),
    sourceReview: input.sourceReview && typeof input.sourceReview === 'object' ? {
      accepted: input.sourceReview.accepted === true,
      baseResumeSha256: String(input.sourceReview.baseResumeSha256 || '').trim(),
      verifiedFactsHash: String(input.sourceReview.verifiedFactsHash || '').trim(),
      requisitionId: String(input.sourceReview.requisitionId || '').trim().slice(0, 160),
      reviewedAt: String(input.sourceReview.reviewedAt || '').trim().slice(0, 40),
    } : null,
    includeCoverLetter: input.includeCoverLetter !== false,
    roleEvidence: {
      location: String(evidenceInput.location || '').trim().slice(0, 180), remote: evidenceInput.remote === true,
      workplaceType: String(evidenceInput.workplaceType || '').trim().slice(0, 80), employmentType: String(evidenceInput.employmentType || '').trim().slice(0, 80),
      salaryMin: Number(evidenceInput.salaryMin) > 0 ? Math.round(Number(evidenceInput.salaryMin)) : null,
      salaryMax: Number(evidenceInput.salaryMax) > 0 ? Math.round(Number(evidenceInput.salaryMax)) : null,
      salaryCurrency: String(evidenceInput.salaryCurrency || 'USD').trim().slice(0, 3).toUpperCase(),
      salaryDisclosure: String(evidenceInput.salaryDisclosure || '').trim().slice(0, 300),
      compensationBasis: ['base', 'ote', 'total compensation', 'unknown'].includes(evidenceInput.compensationBasis) ? evidenceInput.compensationBasis : 'unknown',
      travelDisclosure: String(evidenceInput.travelDisclosure || '').trim().slice(0, 180),
      peopleLeadershipRequired: evidenceInput.peopleLeadershipRequired === true,
      categoryManagementRequired: evidenceInput.categoryManagementRequired === true,
      verifiedAt: String(evidenceInput.verifiedAt || '').trim().slice(0, 40),
    },
  ...
  if (String(input.resumeText || '').trim().length > 50_000) throw new Error('Base resume exceeds the package input limit.');
  const resumeSha256 = createHash('sha256').update(safe.resumeText).digest('hex');
  if (!safe.baseResume) safe.baseResume = { documentId: `legacy-browser:${resumeSha256}`, version: 1,
    sha256: resumeSha256, source: 'legacy-browser', provenance: 'legacy-package-compatibility', reviewState: 'legacy-unverified' };
  if (!safe.baseResume.documentId || !Number.isSafeInteger(safe.baseResume.version) || safe.baseResume.version < 1
    || safe.baseResume.sha256 !== resumeSha256
    || !['applicant-vault', 'legacy-browser'].includes(safe.baseResume.source)
    || !safe.baseResume.provenance || !safe.baseResume.reviewState) throw new Error('An exact selected base resume identity is required.');
  const factsSha256 = createHash('sha256').update(JSON.stringify(safe.verifiedFacts)).digest('hex');
  if (safe.verifiedFactsHash && safe.verifiedFactsHash !== factsSha256) throw new Error('Verified applicant fact versions changed.');
  safe.verifiedFactsHash = factsSha256;
  if (safe.sourceReview && (!safe.sourceReview.accepted || safe.sourceReview.baseResumeSha256 !== safe.baseResume.sha256
    || safe.sourceReview.verifiedFactsHash !== safe.verifiedFactsHash || safe.sourceReview.requisitionId !== safe.requisitionId)) {
    throw new Error('The selected base resume and verified applicant fact versions need a fresh review.');
  }
  if (safe.revision) {
...
export async function readJobAgentRun({ redis, subject, partitionSecret: secret, dataEncryptionKey, runId }) {
  return readJobAgentRunForTenant({ redis, tenantId: jobAgentTenantId(subject, secret), dataEncryptionKey, runId });
}

export async function readJobAgentRunForTenant({ redis, tenantId, dataEncryptionKey, runId }) {
  if (!/^[a-f0-9]{40}$/.test(String(tenantId || ''))) return null;
...
export async function claimJobAgentRun({ redis, runId, dataEncryptionKey, now = new Date(), leaseSeconds = 45 }) {
  ...
  const result = await redis.eval(CLAIM_SCRIPT, [key, dueKey()], [String(record.version), leaseTokenHash, leaseUntil, now.toISOString(), String(RUN_TTL_SECONDS), runId, String(new Date(leaseUntil).getTime()), JSON.stringify(claimEvent), String(Date.parse(record.leaseUntil) || 0), String(Date.parse(record.nextAttemptAt) || 0)]);
...
// Rotate already-served tenants within the existing due index. CAS prevents a
// contending scanner from removing or rescheduling the winning owner's lease.
const ROTATE_DUE_SCRIPT = `
local raw = redis.call('GET', KEYS[1])
if not raw then return redis.call('ZREM', KEYS[2], ARGV[1]) end
local record = cjson.decode(raw)
if tostring(record.version) ~= ARGV[2] then return 0 end
if record.status ~= 'Searching' and record.status ~= 'Preparing' then
  return redis.call('ZREM', KEYS[2], ARGV[1])
end
if record.leaseUntil and record.leaseUntil ~= '' and record.leaseUntil > ARGV[4] then return 0 end
return redis.call('ZADD', KEYS[2], ARGV[3], ARGV[1])
`;

export async function claimNextJobAgentRun({ redis, dataEncryptionKey, now = new Date(), leaseSeconds = 45, servedTenants = new Set() }) {
  const ids = await redis.zrange(dueKey(), 0, now.getTime(), { byScore: true, offset: 0, count: 100 });
  for (const runId of ids || []) {
    const record = parseRecord(await redis.get(runKey(runId)));
    if (!record || !['Searching', 'Preparing'].includes(record.status) || servedTenants.has(record.tenantId)) {
      await redis.eval(ROTATE_DUE_SCRIPT, [runKey(runId), dueKey()], [runId, String(record?.version || ''), String(now.getTime() + 1), now.toISOString()]);
      continue;
    }
    const claimed = await claimJobAgentRun({ redis, runId, dataEncryptionKey, now, leaseSeconds });
    if (claimed) {
      servedTenants.add(claimed.tenantId);
      // Persist rotation even when the invocation's batch limit is one.
      for (const otherId of (ids || []).filter(id => id !== runId)) {
        const other = parseRecord(await redis.get(runKey(otherId)));
        if (other?.tenantId === claimed.tenantId) await redis.eval(ROTATE_DUE_SCRIPT, [runKey(otherId), dueKey()], [otherId, String(other.version), String(now.getTime() + 1), now.toISOString()]);
      }
      return claimed;
    }
    // A failed claim can mean another worker won. Preserve its recovery index.
  }
  return null;
}
...
async function updateLease({ redis, runId, leaseToken, dataEncryptionKey, status, lifecycleState, eventType, result, errorCode = '', nextAttemptAt = '', enqueue = false, heartbeat = false, now = new Date() }) {
  const key = runKey(runId);
  const record = parseRecord(await redis.get(key));
  if (!record || !sameLease(record, leaseToken) || !record.leaseUntil || new Date(record.leaseUntil).getTime() <= now.getTime()) return null;
  ...
  const response = await redis.eval(LEASE_UPDATE_SCRIPT, [key, dueKey()], [
    ...
    String(Date.parse(record.leaseUntil) || 0),
  ]);
...
export async function heartbeatJobAgentRun({ redis, runId, leaseToken, dataEncryptionKey, now = new Date(), leaseSeconds = 45, lifecycleState = null }) {
  ...
  if (!record || !sameLease(record, leaseToken) || !record.leaseUntil || new Date(record.leaseUntil).getTime() <= now.getTime()) return null;
  const leaseUntil = new Date(now.getTime() + Math.max(15, Math.min(120, leaseSeconds)) * 1000).toISOString();
  // Redis/Lua round trips may omit an empty result envelope. EVAL accepts the
  // JSON string "null", but rejects an undefined argument serialized as null.
  const response = await redis.eval(LEASE_UPDATE_SCRIPT, [key, dueKey()], [String(record.version), record.leaseTokenHash, record.status, now.toISOString(), leaseUntil, record.leaseTokenHash, JSON.stringify(record.resultEnvelope ?? null), record.lastErrorCode || '', record.nextAttemptAt || '', String(RUN_TTL_SECONDS), 'enqueue', String(new Date(leaseUntil).getTime()), runId, lifecycleState || record.lifecycleState || record.status, now.toISOString(), JSON.stringify({ id: `${record.operationId || runId}:heartbeat:${record.version + 1}`, type: 'WORKER_HEARTBEAT', state: lifecycleState || record.lifecycleState || record.status, at: now.toISOString(), attempt: Number(record.attempt) || 0 }), String(Date.parse(record.leaseUntil) || 0)]);
  return Array.isArray(response) && response[0] === 'updated' ? publicRun(parseRecord(response[1]), { dataEncryptionKey }) : null;
}
