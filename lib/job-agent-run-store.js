...
export function validateApplicationPackageInput(input = {}) {
  ...
  try {
    const url = new URL(safe.directEmployerUrl);
    if (url.protocol !== 'https:') throw new Error('protocol');
  } catch { throw new Error('A verified HTTPS direct-employer URL is required.'); }
...
function publicRun(record, config) {
  ...
  return {
    ...
    nextAttemptAt: record.nextAttemptAt || null, leaseUntil: record.leaseUntil || null,
...
async function createJobAgentRunForResolvedTenant({ redis, tenantId, dataEncryptionKey, mission, taskType = 'direct_employer_discovery', idempotencyKey: idem, now = new Date() }) {
  ...
  const record = {
    version: 1, id, tenantId, taskType, status: 'Searching', attempt: 0, maxAttempts: MAX_ATTEMPTS,
    createdAt: timestamp, updatedAt: timestamp, nextAttemptAt: timestamp, leaseUntil: '', leaseTokenHash: '', lastErrorCode: '',
...
export async function claimJobAgentRun({ redis, runId, dataEncryptionKey, now = new Date(), leaseSeconds = 45 }) {
  ...
  const result = await redis.eval(CLAIM_SCRIPT, [key, dueKey()], [String(record.version), leaseTokenHash, leaseUntil, now.toISOString(), String(RUN_TTL_SECONDS), runId, String(new Date(leaseUntil).getTime())]);
...
async function updateLease({ redis, runId, leaseToken, dataEncryptionKey, status, result, errorCode = '', nextAttemptAt = '', enqueue = false, now = new Date() }) {
  const key = runKey(runId);
  const record = parseRecord(await redis.get(key));
...
export async function heartbeatJobAgentRun({ redis, runId, leaseToken, dataEncryptionKey, now = new Date(), leaseSeconds = 45 }) {
  const key = runKey(runId);
  const record = parseRecord(await redis.get(key));
  if (!record || !sameLease(record, leaseToken)) return null;
  const leaseUntil = new Date(now.getTime() + Math.max(15, Math.min(120, leaseSeconds)) * 1000).toISOString();
  const response = await redis.eval(LEASE_UPDATE_SCRIPT, [key, dueKey()], [String(record.version), record.leaseTokenHash, record.status, now.toISOString(), leaseUntil, record.leaseTokenHash, JSON.stringify(record.resultEnvelope), record.lastErrorCode || '', record.nextAttemptAt || '', String(RUN_TTL_SECONDS), 'enqueue', String(new Date(leaseUntil).getTime()), runId]);
  return Array.isArray(response) && response[0] === 'updated' ? publicRun(parseRecord(response[1]), { dataEncryptionKey }) : null;
}

export function retryDelaySeconds(attempt) {
  return Math.min(60 * 60, 30 * (2 ** Math.max(0, Number(attempt) - 1)));
}

export async function finishJobAgentRun(input) {
  return updateLease({ ...input, status: 'Finished', result: input.result, now: input.now || new Date() });
}

export async function waitForUserJobAgentRun(input) {
  return updateLease({ ...input, status: 'Waiting for You', result: input.result, errorCode: input.reasonCode || 'HUMAN_ACTION_REQUIRED', now: input.now || new Date() });
}
...
export async function failJobAgentRun({ redis, runId, leaseToken, dataEncryptionKey, errorCode = 'TRANSIENT_FAILURE', retryable = true, now = new Date() }) {
  ...
  return updateLease({ redis, runId, leaseToken, dataEncryptionKey, status: canRetry ? 'Searching' : 'Failed', errorCode: String(errorCode).slice(0, 80), nextAttemptAt: next, enqueue: canRetry, now });
}
...
export async function setJobAgentRunStatus({ redis, subject, partitionSecret: secret, dataEncryptionKey, runId, status, now = new Date() }) {
  ...
  const response = await redis.eval(USER_UPDATE_SCRIPT, [key, dueKey()], [String(record.version), status, now.toISOString(), nextAttemptAt, String(RUN_TTL_SECONDS), runId, status === 'Searching' ? 'enqueue' : '', String(now.getTime()), record.status === 'Failed' && status === 'Searching' ? 'reset' : '']);
  return Array.isArray(response) && response[0] === 'updated' ? publicRun(parseRecord(response[1]), { dataEncryptionKey }) : null;
}
