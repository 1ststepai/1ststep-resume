...
if KEYS[5] then
  ...
  if not raw then return {'schedule_lease_lost'} end
  local schedule = cjson.decode(raw)
  local serverTime = redis.call('TIME')
  local serverNow = tonumber(serverTime[1]) * 1000 + math.floor(tonumber(serverTime[2]) / 1000)
  if schedule.status ~= 'active' or schedule.leaseTokenHash ~= ARGV[6]
    or schedule.version ~= tonumber(ARGV[7]) or schedule.leaseUntil ~= ARGV[8]
    or tonumber(ARGV[9]) <= serverNow then return {'schedule_lease_lost'} end
end
...
async function createJobAgentRunForResolvedTenant({ redis, tenantId, dataEncryptionKey, mission, taskType = 'direct_employer_discovery', idempotencyKey: idem, scheduleClaim, now = new Date() }) {
  ...
  if (scheduleClaim !== undefined) {
    if (taskType !== 'direct_employer_discovery' || !scheduleClaim?.leaseToken
      || !Number.isSafeInteger(scheduleClaim.version) || scheduleClaim.version < 1
      || !Number.isFinite(Date.parse(scheduleClaim.leaseUntil))) throw new Error('A valid schedule claim is required.');
    // Fence cancellation/edit/reclaim in the same transaction as queue admission.
    keys.push(`1ststep:job-agent-schedule:v1:tenant:${tenantId}`);
    args.push(createHash('sha256').update(scheduleClaim.leaseToken).digest('hex'), String(scheduleClaim.version),
      scheduleClaim.leaseUntil, String(Date.parse(scheduleClaim.leaseUntil)));
  }
  const result = await redis.eval(CREATE_SCRIPT, keys, args);
  const [status, returnedId] = Array.isArray(result) ? result : ['error', ''];
  if (status === 'schedule_lease_lost') throw new Error('SCHEDULE_LEASE_LOST');
