...
import { cloudflareR2Configuration } from './cloudflare-r2-private-storage.js';
...
export function jobAgentObjectStorageConfiguration(env = process.env, blobClient = { put, get, del, list }) {
  ...
  if (ownerReviewedCapabilityCeilingApplies(env)) {
    return { mode: 'unavailable', production: isProduction, ready: false, reason: 'OWNER_REVIEWED_CAPABILITY_CEILING', scanner };
  }
  ...
  const provider = String(env.JOB_AGENT_OBJECT_STORAGE_PROVIDER || 'vercel-blob').toLowerCase();
  if (provider === 'cloudflare-r2') {
    const r2 = cloudflareR2Configuration(env);
    if (!r2.ready) return { mode: 'unavailable', production: isProduction, ready: false, reason: r2.reason, scanner };
    ...
    return { mode: 'cloudflare-r2-private', production: isProduction, ready: true, blobClient: r2.client, scanner };
  }
  if (provider !== 'vercel-blob') return { mode: 'unavailable', production: isProduction, ready: false, reason: 'OBJECT_STORAGE_PROVIDER_UNSUPPORTED', scanner };
...
function privateStorageReady(configuration) {
  return configuration?.ready === true && ['vercel-blob-private', 'cloudflare-r2-private'].includes(configuration.mode);
}
...
export async function persistApplicationPackageArtifacts({ artifacts, tenantId, runId, dataEncryptionKey, redis = null, configuration = jobAgentObjectStorageConfiguration(), env = process.env, fetchImpl = fetch, now = new Date() }) {
  ...
  if (!privateStorageReady(configuration)) throw new Error('PRIVATE_OBJECT_STORAGE_NOT_CONFIGURED');
  ...
  try {
    for (const artifact of artifacts) {
      ...
      const storedArtifact = {
        ...
        objectRef: { provider: configuration.mode, pathname: result.pathname || pathname },
...
export async function readApplicationPackageArtifact({ artifact, tenantId, runId, dataEncryptionKey, configuration = jobAgentObjectStorageConfiguration() }) {
  ...
  if (!privateStorageReady(configuration) || artifact?.objectRef?.provider !== configuration.mode) throw new Error('PRIVATE_OBJECT_STORAGE_NOT_CONFIGURED');
...
export async function processExpiredApplicationPackageArtifacts({ redis, configuration = jobAgentObjectStorageConfiguration(), now = new Date(), limit = 25 }) {
  if (!redis || !privateStorageReady(configuration)) return { deleted: 0, status: configuration.production ? 'not-configured' : 'development-disabled' };
