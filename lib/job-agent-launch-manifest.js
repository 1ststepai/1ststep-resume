...
import { jobAgentDiscordRelayConfiguration, publicJobAgentDiscordRelayConfiguration } from './job-agent-discord-relay.js';
...
function encryptionRuntimeConfigured(env, production) {
...
function durableRuntimeConfigured(env, storage, production) {
  if (!encryptionRuntimeConfigured(env, production)) return false;
...
export function jobAgentLaunchManifest(env = process.env, { now = new Date() } = {}) {
  ...
  const discordAlerts = jobAgentDiscordRelayConfiguration(env);
  ...
  if (!policy.counselApproved || !jobAgentConsentEnforcementRequired(env)) signedBetaBlockers.push('COUNSEL_APPROVED_CONSENT_NOT_CONFIGURED');
  ...
  const ownerReviewedBetaBlockers = [];
  if (production) ownerReviewedBetaBlockers.push('OWNER_REVIEWED_POLICY_NOT_ALLOWED_IN_PRODUCTION');
  if (!encryptionRuntimeConfigured(env, production)) ownerReviewedBetaBlockers.push('DURABLE_RUNTIME_NOT_CONFIGURED');
  if (!jobAgentConsentEnforcementRequired(env)) ownerReviewedBetaBlockers.push('CONSENT_ENFORCEMENT_NOT_CONFIGURED');
  if (!policy.ownerReviewed || !policy.ready || policy.ownerCounselConflict) ownerReviewedBetaBlockers.push('OWNER_REVIEWED_POLICY_NOT_CONFIGURED');
  if (!pilotAdmission.ready || pilotAdmission.maxUsers !== 5) ownerReviewedBetaBlockers.push('PILOT_ADMISSION_CONTROL_NOT_CONFIGURED');
  if (!entitlementPolicy.ready) ownerReviewedBetaBlockers.push('JOB_AGENT_ACCESS_POLICY_NOT_CONFIGURED');
  const ownerReviewedControlledBeta = capability(ownerReviewedBetaBlockers.length === 0, ownerReviewedBetaBlockers);
  ...
  const currentMode = finalSubmission.eligible ? 'final-submission'
    : assistedApplication.eligible ? 'assisted-application'
      : packageReady.eligible ? 'package-ready'
        : signedBeta.eligible ? 'signed-beta'
          : ownerReviewedControlledBeta.eligible ? 'owner-reviewed-controlled-beta' : 'preview';

  const capabilities = {
    preview: capability(true),
    ownerReviewedControlledBeta,
    signedBeta,
    packageReady,
    assistedApplication,
    finalSubmission,
  ...
  return {
    ...
    discordOperatorAlerts: publicJobAgentDiscordRelayConfiguration(discordAlerts),
