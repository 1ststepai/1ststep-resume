...
export function jobAgentConsentPolicyConfiguration(env = process.env) {
  ...
  const ready = versionsReady && (counselApproved || ownerReviewed.valid);
  return {
    ready,
    counselApproved,
...
export function activeJobAgentConsent(existing, policy) {
  ...
  if (!policy?.ready || record?.status !== 'active') {
    const unconfigured = policy?.ownerReviewedRequested && !policy?.counselApproved
...
export function publicJobAgentConsent(existing, policy) {
  ...
  return {
    ...
    approvalSource: policy?.approvalSource || null,
    counselApproved: policy?.counselApproved === true,
