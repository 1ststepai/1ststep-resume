...
export const APPLICATION_ACTIVITY_STATUSES = Object.freeze([
  ...
  'Human Action Required - password/reset', 'Human Action Required - OTP delivery and code',
...
export const REDACTED_WORKFLOW_REPLAY = Object.freeze([
  ...
  { employer: 'Holley Performance Brands', requisitionId: '1552', status: 'Human Action Required - OTP delivery and code', evidence: 'ADP accepted the authorized profile transmission and opened a verification dialog with masked SMS/email destinations. Preserve the dialog; never record the code.' },
]);
...
const nextStatuses = Object.freeze({
  ...
  Verified: ['Verified - Package Preparation', 'Blocked'],
  'Verified - Package Preparation': ['Package Ready', 'Blocked'],
...
export function recordGeneratedPackage(inputState, roleId, input, at) {
  ...
  if (!['Verified', 'Verified - Package Preparation', 'Package Ready'].includes(role.status)) throw new Error('A role must be Verified before generating its package.');
  ...
  const nextStatus = role.status === 'Verified' ? 'Verified - Package Preparation' : role.status;
  const updated = {
    ...role, packageDraft, status: nextStatus,
    statusHistory: nextStatus === role.status ? role.statusHistory : [...role.statusHistory, { status: nextStatus, reason: 'Role-specific package preparation started', at: nowIso(at) }],
    updatedAt: nowIso(at),
  };
...
export function startManagedApplicationSession(inputState, input = {}, at) {
  ...
  const session = {
    ...
    autonomyLevel: AUTONOMY_LEVELS.includes(input.autonomyLevel) ? input.autonomyLevel : state.autonomy.level,
    ...
    stepIndex: 0, step, status: 'active', agentStatus: 'Checking the direct employer requisition',
...
export function pauseManagedApplicationSession(inputState, sessionId, reason = 'browser-timeout', at) {
  ...
  const labels = {
    ...
    'otp-delivery': 'Employer verification dialog preserved with masked delivery choices. A person must choose delivery and enter the code privately.',
  };
  ...
  const blockerType = reason === 'login-required' ? 'LOGIN'
    : reason === 'ats-configuration' ? 'ATS_CONFIGURATION'
      : reason === 'invalid-credentials' ? 'PASSWORD_RESET'
        : reason === 'otp-delivery' ? 'OTP'
          : 'BROWSER_TIMEOUT';
  const updated = { ...session, status: 'paused', agentStatus: summary, updatedAt: nowIso(at), blockers: [...session.blockers, { id: makeId('blocker'), type: blockerType, status: 'open', summary }], timeline: [...session.timeline, workflowTimelineEvent(session.step, 'recovery-required', summary, at)] };
  state = { ...state, applicationSessions: state.applicationSessions.map(item => item.id === sessionId ? updated : item) };
