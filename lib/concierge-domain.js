...
export const APPLICATION_ACTIVITY_STATUSES = Object.freeze([
  ...
  'Human Action Required - password/reset', 'Human Action Required - OTP',
...
export const REDACTED_WORKFLOW_REPLAY = Object.freeze([
  ...
  { employer: 'Holley Performance Brands', requisitionId: '1552', status: 'Human Action Required - OTP', evidence: 'Candidate selected masked email delivery; ADP confirmed a verification code was sent and preserved the Enter the Verification Code dialog. Never record or display the code.' },
...
export function pauseManagedApplicationSession(inputState, sessionId, reason = 'browser-timeout', at) {
  ...
  const labels = {
    ...
    'otp-delivery': 'Email delivery selected; employer confirmed a verification code was sent and preserved the code-entry dialog. The code is never stored or displayed.',
  ...
  const checkpointStatus = reason === 'invalid-credentials' ? 'Human Action Required - password/reset'
    : reason === 'otp-delivery' ? 'Human Action Required - OTP'
