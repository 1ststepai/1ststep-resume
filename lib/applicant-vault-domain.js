...
export function createApplicantVault(input = {}) {
  const vault = {
    ...
    selectedBaseResume: input.selectedBaseResume || null,
...
export function upsertVaultFact(inputVault, input = {}, at) {
  ...
  const factVersion = {
    ...
    originKind: text(input.originKind, 32) || 'unknown',
...
export function syncCanonicalApplicantProfile(inputVault, input = {}, at) {
  ...
  for (const fact of facts) {
    ...
    const normalized = {
      ...
      originKind: text(fact?.originKind, 32) || 'unknown',
    ...
    const unchanged = current
      ...
      && (current.originKind || 'unknown') === normalized.originKind
...
export function verifiedVaultFacts(inputVault) {
  const vault = createApplicantVault(inputVault);
  return vault.facts.filter(fact => fact.status === 'active').flatMap(fact => {
    const version = fact.versions.find(item => item.version === fact.currentVersion);
    if (!version || version.revokedAt || version.scope?.memory || version.autoReuse !== true
      || version.sensitivity !== 'standard' || CONSEQUENTAL_FIELDS.has(fact.fieldKey)
      || !ALLOWED_VERIFICATION.has(version.verificationState)
      || /generated|parsed|inferred|legacy|readiness-interview|saved-resume-section|career-story-proposal/i.test(String(version.originKind || '') + ' ' + String(version.provenance || ''))) return [];
    return [{ id: fact.id, fieldKey: fact.fieldKey, version: version.version, value: version.value,
      provenance: version.provenance, verificationState: version.verificationState }];
  }).sort((a, b) => (a.fieldKey + ':' + a.id).localeCompare(b.fieldKey + ':' + b.id));
}

export function verifiedVaultFactsHash(inputVault) {
  return createHash('sha256').update(JSON.stringify(verifiedVaultFacts(inputVault))).digest('hex');
}

export function selectVaultBaseResume(inputVault, input = {}, at) {
  const vault = createApplicantVault(inputVault);
  requireConsent(vault, 'documents');
  const document = vault.documents.find(item => item.id === text(input.documentId, 128)
    && item.type === 'master-resume' && item.status === 'active');
  const versionNumber = Number(input.version);
  const version = document?.versions.find(item => item.version === versionNumber);
  if (!version || !/^[a-f0-9]{64}$/.test(String(version.sha256 || ''))
    || createHash('sha256').update(version.text).digest('hex') !== version.sha256
    || input.reviewed !== true) throw new Error('Review and select an exact active master resume version.');
  const selectedBaseResume = {
    documentId: document.id, version: versionNumber, sha256: version.sha256,
    source: 'applicant-vault', provenance: version.provenance,
    factsHash: verifiedVaultFactsHash(vault), selectedAt: iso(at), reviewState: 'candidate-reviewed',
  };
  if (vault.selectedBaseResume && Object.entries(selectedBaseResume).every(([key, value]) => key === 'selectedAt' || vault.selectedBaseResume[key] === value)) return vault;
  return validateApplicantVault({ ...vault, selectedBaseResume, updatedAt: selectedBaseResume.selectedAt,
    audit: [...vault.audit, auditEvent('BASE_RESUME_SELECTED', document.id, at, { version: versionNumber, sha256: version.sha256 })] });
}
...
export function validateApplicantVault(input) {
  ...
  if (vault.selectedBaseResume !== null) {
    const selected = vault.selectedBaseResume;
    if (typeof selected !== 'object' || Array.isArray(selected) || !text(selected.documentId, 128)
      || !Number.isSafeInteger(selected.version) || selected.version < 1
      || !/^[a-f0-9]{64}$/.test(String(selected.sha256 || ''))
      || !/^[a-f0-9]{64}$/.test(String(selected.factsHash || ''))
      || selected.source !== 'applicant-vault' || selected.reviewState !== 'candidate-reviewed'
      || !text(selected.provenance, 160) || Number.isNaN(new Date(selected.selectedAt).getTime())) {
      throw new Error('Selected base resume is invalid.');
    }
  }
...
export function publicVaultSummary(inputVault) {
  const vault = createApplicantVault(inputVault);
  return {
    schemaVersion: vault.schemaVersion, consent: vault.consent, updatedAt: vault.updatedAt, selectedBaseResume: vault.selectedBaseResume,
