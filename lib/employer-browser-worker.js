...
import { ownerReviewedCapabilityCeilingApplies } from './job-agent-owner-reviewed-policy.js';
...
export function employerBrowserWorkerConfiguration(env = process.env) {
  if (ownerReviewedCapabilityCeilingApplies(env)) return { enabled: false, reason: 'OWNER_REVIEWED_CAPABILITY_CEILING' };
...
function action(session, type, field, summary, metadata = {}) {
  const proposal = (session?.proposedFields || []).find(item => item.fieldKey === field.fieldKey);
  return {
    type, fieldRef: field.fieldRef, fieldKey: field.fieldKey, summary,
    employer: String(session?.role?.employer || ''), role: String(session?.role?.title || ''),
    requisition: String(session?.role?.requisitionId || ''), currentStep: 'employer_form',
    exactBlocker: field.label, why: summary, suggestedAnswer: null,
    sourceFact: proposal?.factId ? { factId: proposal.factId, provenance: proposal.provenance || null, confidence: Number(proposal.confidence) || 0 } : null,
    primaryAction: 'Resolve',
    ...(type === 'AMBIGUOUS_FACT' ? { question: field.label } : {}), ...metadata,
  };
}
...
export function planEmployerFormStep({ session, pageUrl, fields = [], now = new Date() } = {}) {
  ...
  for (const field of normalized) {
    ...
    if (field.inputType === 'password' || CREDENTIAL.test(semantic)) actions.push(action(session, 'LOGIN', field, 'Sign in directly on the verified employer page. Credentials are never collected by 1stStep.'));
    else if (field.inputType === 'file' || DOCUMENT_UPLOAD.test(semantic)) actions.push(action(session, 'DOCUMENT_UPLOAD', field, 'Upload the exact approved résumé or cover-letter version on the verified employer page.'));
    else if (OTP.test(semantic)) actions.push(action(session, 'OTP', field, 'Enter the latest verification code directly on the employer page.'));
    else if (field.inputType === 'captcha' || CAPTCHA.test(semantic)) actions.push(action(session, 'CAPTCHA', field, 'Complete the security challenge directly on the employer page.'));
    else if (IDENTITY.test(semantic)) actions.push(action(session, 'IDENTITY_VERIFICATION', field, 'Complete identity verification directly with the employer.'));
    else if (OUTSIDE_EMPLOYMENT.test(semantic)) actions.push(action(session, 'OUTSIDE_EMPLOYMENT_CONFLICT', field, 'Review this employer-specific conflict or restrictive-agreement question.'));
    else if (CERTIFICATION.test(semantic)) actions.push(action(session, 'NONSTANDARD_CERTIFICATION', field, 'Review this certification or attestation before continuing.'));
    else if (field.required && ELIGIBILITY_SCREEN.test(semantic)) actions.push(action(session, 'AMBIGUOUS_FACT', field, 'Potential eligibility screen: answer the exact employer question truthfully. An unsupported answer may end the application; you can skip this job instead.', { riskCategory: 'eligibility-screening', canSkipJob: true }));
    else if (PROTECTED_OR_CONSEQUENTIAL.test(semantic)) {
      if (field.required) actions.push(action(session, 'AMBIGUOUS_FACT', field, 'Answer this consequential employer question explicitly; the agent will not infer it.'));
      ...
      } else if (field.required) actions.push(action(session, 'AMBIGUOUS_FACT', field, 'Confirm this required answer before the agent fills it.'));
  ...
  return {
    ...
    stagedFields, leftUnanswered, actions, nextAction: actions[0] || null,
