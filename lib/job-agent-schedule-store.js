...
import { readJobAgentLearningStateForTenant } from './job-agent-learning-store.js';
import { applyLearnedPreferencesToMission } from './job-agent-learning-domain.js';
...
export async function claimNextJobAgentSchedule({ redis, dataEncryptionKey, now = new Date() }) {
  ...
  for (const tenantId of tenantIds || []) {
    ...
    if (Array.isArray(result) && result[0] === 'claimed') return { tenantId, leaseToken, leaseUntil: leaseUntil.toISOString(), schedule: publicSchedule(parse(result[1]), dataEncryptionKey) };
  }
  return null;
}
...
export async function processNextJobAgentSchedule({ redis, partitionSecret, dataEncryptionKey, env = process.env, now = new Date() }) {
  ...
  try {
    ...
    const learned = await readJobAgentLearningStateForTenant({ redis, tenantId: claimed.tenantId, dataEncryptionKey }).catch(() => ({ state: null }));
    const mission = learned.state ? applyLearnedPreferencesToMission(learned.state, claimed.schedule.mission) : claimed.schedule.mission;
    const created = await createJobAgentRunForTenant({
      redis, tenantId: claimed.tenantId, dataEncryptionKey, mission,
      scheduleClaim: { leaseToken: claimed.leaseToken, version: claimed.schedule.version, leaseUntil: claimed.leaseUntil },
    ...
    if (error?.message === 'SCHEDULE_LEASE_LOST') return { status: 'deferred', reason: 'SCHEDULE_LEASE_LOST' };
