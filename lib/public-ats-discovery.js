...
import { jobMatchesHardExclusion, jobTitleMatchesMission } from './job-mission-relevance.js';
...
export function publicDiscoveryRuntimeOptions(input = {}) {
  return Object.freeze({
    ...
    detailConcurrency: boundedInteger(input.detailConcurrency, 8, 1, 16),
    totalTimeoutMs: boundedInteger(input.totalTimeoutMs, 32_000, 1_000, 35_000),
  });
}
...
function workSetting(location, workplaceType = '', isRemote) {
  const place = text(location);
  const mode = text(workplaceType);
  const remoteInPlace = /\bremote\b/i.test(place);
  const remoteInMode = /\bremote\b/i.test(mode);
  const negated = /\b(?:not|no|without)\s+remote\b|\bnon[-\s]?remote\b|\bremote\s+(?:not\s+(?:available|offered|allowed|permitted)|unavailable)\b/i.test(`${place} ${mode}`);
  const hybrid = /\bhybrid\b/i.test(`${place} ${mode}`);
  const onSiteInPlace = /\b(?:on[- ]?site|in[- ]?office)\b/i.test(place);
  const onSiteInMode = /\b(?:on[- ]?site|in[- ]?office)\b/i.test(mode);
  const conflict = negated && (remoteInPlace || remoteInMode || isRemote === true)
    || isRemote === false && (remoteInPlace || remoteInMode)
    || isRemote === true && onSiteInPlace
    || onSiteInMode && (remoteInPlace || isRemote === true)
    || remoteInMode && onSiteInPlace;
  if (conflict) return { remote: false, workplaceType: 'Unknown' };
  if (hybrid) return { remote: false, workplaceType: 'Hybrid' };
  if (onSiteInMode || isRemote === false || negated) return { remote: false, workplaceType: mode || 'Unknown' };
  const remote = isRemote === true || remoteInMode || remoteInPlace;
  return { remote, workplaceType: remote ? 'Remote' : mode || 'Unknown' };
}
...
function normalizeGreenhouse(source, payload) {
  return (payload?.jobs || []).map(job => {
    ...
    const setting = workSetting(location);
    return {
...
function normalizeLever(source, payload) {
  return (Array.isArray(payload) ? payload : []).map(job => {
    ...
    const setting = workSetting(location, job.workplaceType);
    return {
...
function normalizeAshby(source, payload) {
  ...
  return (payload?.jobs || []).filter(job => job.isListed !== false).map(job => {
    ...
    const setting = workSetting(location, job.workplaceType, typeof job.isRemote === 'boolean' ? job.isRemote : undefined);
    return {
      provider: source.provider, sourceSlug: source.slug, employer: source.employer, title: text(job.title), requisitionId: text(job.id),
...
export async function fetchPublicAtsJson({ descriptor, url, fetchImpl = fetch, timeoutMs = 8_000, now = Date.now(), signal } = {}) {
  ...
  const requestSignal = AbortSignal.timeout(Math.max(1_000, Math.min(15_000, Number(timeoutMs) || 8_000)));
...
export async function discoverPublicJobs({ mission = {}, sources = [], fetchImpl = fetch, limit = 50, now = new Date(), runtime = {} } = {}) {
  ...
  const signal = AbortSignal.timeout(runtimeOptions.totalTimeoutMs);
  ...
  const outcomes = await settleWithConcurrency(safeSources, async source => {
    ...
    const attempts = await settleWithConcurrency(urls, url => fetchPublicAtsJson({
      descriptor, url, fetchImpl, timeoutMs: runtimeOptions.requestTimeoutMs, signal,
  ...
  const enriched = await settleWithConcurrency(candidates, async job => {
    ...
    let payload;
    try {
      const detail = await fetchPublicAtsJson({ descriptor, url: detailUrl, fetchImpl, timeoutMs: runtimeOptions.detailTimeoutMs, signal });
  ...
  }, runtimeOptions.detailConcurrency);
  const detailed = [];
  ...
  return {
    ...
    partial: signal.aborted || errors.length > 0,
