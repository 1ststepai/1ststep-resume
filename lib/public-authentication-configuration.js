...
const PRODUCTION_CLERK_FRONTEND_API_URL = 'https://clerk.1ststep.ai';
...
export function clerkBrowserConfiguration(publishableKey, vercelEnvironment) {
  ...
  try {
    ...
    if (frontendApiUrl.protocol !== 'https:' || frontendApiUrl.pathname !== '/' || frontendApiUrl.port) return null;
    const production = vercelEnvironment === 'production';
    if (production && (match[1] !== 'pk_live' || frontendApiUrl.origin !== PRODUCTION_CLERK_FRONTEND_API_URL)) return null;
    if (!production && (match[1] !== 'pk_test' || !frontendApiUrl.hostname.endsWith('.clerk.accounts.dev'))) return null;
    return {
      frontendApiUrl: frontendApiUrl.origin,
