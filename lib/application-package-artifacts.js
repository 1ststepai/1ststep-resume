...
async function buildPdf({ pages }) {
  const document = new PDFDocument({
    autoFirstPage: false, size: 'LETTER',
    margins: { top: PDF_MARGIN, right: PDF_MARGIN, bottom: PDF_MARGIN, left: PDF_MARGIN },
    compress: false,
    // Stable metadata makes a candidate-reviewed document reproducible across the
    // prepare and download calls of a short-lived extension handoff.
    info: {
      Author: '1stStep.ai Job Agent', Creator: '1stStep.ai Job Agent', Producer: '1stStep.ai Job Agent',
      CreationDate: new Date(0), ModDate: new Date(0),
    },
  });
  const chunks = [];
