...
export async function buildCompleteAccountDataExport({ config, subject, now = new Date() }) {
  ...
  const [consent, schedule, notifications, emailSuppression, vault, learning, campaign, capturedJobs, runs, sessions, browserTasks, submissionTasks, receiptTasks, browserSessions] = await Promise.all([
  ...
  return buildAccountDataExport({
    subject, consent, schedule, notifications, emailSuppression, vault, learning, campaign, capturedJobs: capturedJobs.items, now,
