...
import { listCapturedJobs } from './captured-job-store.js';
import { exportPartnerAccount } from './partner-account.js';
...
export async function buildCompleteAccountDataExport({ config, subject, now = new Date() }) {
  ...
  const [consent, schedule, notifications, emailSuppression, vault, learning, partnerAccount, campaign, capturedJobs, runs, sessions, browserTasks, submissionTasks, receiptTasks, browserSessions] = await Promise.all([
    ...
    exportPartnerAccount({ ...config, secret: config.partitionSecret, subject }),
    readTenantCampaignState({ ...config, subject }), page(listCapturedJobs), page(listJobAgentRuns), page(listDurableApplicationSessions),
  ...
  const collections = { capturedJobs, jobAgentRuns: runs, applicationSessions: sessions, employerBrowserTasks: browserTasks, applicationSubmissionTasks: submissionTasks, applicationReceiptTasks: receiptTasks, employerBrowserSessions: browserSessions };
  ...
  return buildAccountDataExport({
    subject, consent, schedule, notifications, emailSuppression, vault, learning, partnerAccount, campaign, capturedJobs: capturedJobs.items, now,
