...
export async function buildCompleteAccountDataExport({ config, subject, now = new Date() }) {
  ...
  const [consent, schedule, notifications, emailSuppression, vault, campaign, runs, sessions, browserTasks, submissionTasks, receiptTasks, browserSessions] = await Promise.all([
  ...
  return buildAccountDataExport({
    subject, consent, schedule, notifications, emailSuppression, vault, campaign, now,
