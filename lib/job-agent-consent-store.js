...
export async function jobAgentConsentGate(config, subject, env = process.env) {
  ...
  const policyConfigurationFailure = ['JOB_AGENT_POLICY_NOT_CONFIGURED', 'JOB_AGENT_OWNER_REVIEWED_POLICY_NOT_CONFIGURED', 'JOB_AGENT_OWNER_COUNSEL_POLICY_CONFLICT', 'JOB_AGENT_PILOT_NOT_CONFIGURED'].includes(result.code);
  ...
  return {
    ...
    error: policyConfigurationFailure
      ? result.code === 'JOB_AGENT_PILOT_NOT_CONFIGURED' ? 'Controlled-beta admission is not configured.'
        : result.code === 'JOB_AGENT_OWNER_COUNSEL_POLICY_CONFLICT' ? 'Owner-reviewed Job Agent policy cannot be combined with counsel approval.'
        : result.code === 'JOB_AGENT_OWNER_REVIEWED_POLICY_NOT_CONFIGURED' ? 'Owner-reviewed Job Agent policy is not bound to the exact served documents.'
        : 'Counsel-approved Job Agent policy versions are not configured.'
      : pilotInviteRequired
